import{o as e,s as t}from"./chem-format-5TOrh_IH.js";import{a as n,c as r,d as i,h as a,n as o,o as s,r as c,t as l}from"./html-phieu-orjDjTvL.js";var u={sai:`Ở NHÀ LÀM SAI`,chuaLam:`Ở NHÀ CHƯA LÀM`,dung:`Ở NHÀ LÀM ĐÚNG`},d=12,f=[`A`,`B`,`C`,`D`],p=[`a`,`b`,`c`,`d`];function m(e){return`${String(e.getDate()).padStart(2,`0`)}/${String(e.getMonth()+1).padStart(2,`0`)}/${e.getFullYear()}`}function h(e){return e?e.replace(/\r\n?/g,`
`).replace(/<br\s*\/?\s*>/gi,`
`).split(`
`).map(e=>n(e)).join(`<br>`):``}function g(e){if(!e)return!1;let t=e.text||``,n=!!(e.anhThanCau||e.anhLuaChon&&e.anhLuaChon.some(Boolean)||/<img\b/i.test(t)),r=!!e.bang||/<table\b/i.test(t);if((n||r)&&(t.length>90||e.phan===`I`&&e.luaChon&&e.luaChon.some(e=>(e||``).length>30)||e.phan===`II`)||t.length>280||t.split(/\r\n|\r|\n|<br\s*\/?>/i).length>=4)return!0;if(e.phan===`II`&&e.luaChon&&e.luaChon.length>0){let n=e.luaChon.reduce((e,t)=>e+(t||``).length,0);if(t.length+n>320||e.luaChon.some(e=>(e||``).length>80))return!0}else if(e.phan===`I`&&e.luaChon&&e.luaChon.length>0){let n=e.luaChon.reduce((e,t)=>e+(t||``).length,0);if(t.length+n>360||e.luaChon.some(e=>(e||``).length>65))return!0}else if(e.phan===`III`&&t.length>250)return!0;return!1}function _(n){let i=[];if(n.anhThanCau?i.push(o(n.anhThanCau,`mc-anh`,`Ảnh đề bài`)):i.push(`<div class="mc-de">${h(n.text)}</div>`),n.text?(i.push(e(n.text)),i.push(t(n.text,r(n,`sau_de`)))):i.push(r(n,`sau_de`)),i.push(c(n.bang)),n.phan===`I`&&n.luaChon&&n.luaChon.length>0){let e=n.luaChon.map((e,t)=>{let i=n.anhLuaChon?.[t],a=i?o(i,`mc-anh-pa`,`Phương án ${f[t]}`):h(e);return`<div class="mc-pa"><span class="mc-ky">${f[t]??t+1}</span><span class="mc-pa-chu">${a}</span>${r(n,`sau_pa_${f[t]}`)}</div>`});i.push(`<div class="mc-ds-pa mc-auto-options">${e.join(``)}</div>`)}else if(n.phan===`II`&&n.luaChon&&n.luaChon.length>0){let e=n.luaChon.map((e,t)=>{let i=n.anhLuaChon?.[t],a=i?o(i,`mc-anh-pa`,`Ý ${p[t]}`):h(e);return`<div class="mc-pa"><span class="mc-ky">${p[t]??t+1}</span><span class="mc-pa-chu">${a}</span></div>${r(n,`sau_y_${p[t]}`)}`});i.push(`<div class="mc-ds-pa">${e.join(``)}</div>`)}else n.phan===`III`&&i.push(`<div class="mc-ngan">Trả lời ngắn — em viết kết quả và trình bày các bước lên bảng.</div>`);return i.push(r(n,`cuoi_cau`)),i.filter(Boolean).join(``)}function v(e){let t=e.btvnCau?`<span class="mc-btvn-the mc-btvn-${e.btvnCau}">${u[e.btvnCau]}</span>`:``,n=e.btvnTom,r=n&&n.soCauGiao>0?`<span class="mc-btvn-so">Về nhà: làm ${n.soDaLam}/${n.soCauGiao} câu · đúng ${n.soDung} · sai ${n.soSai} · chưa làm ${n.soChuaLam}</span>`:``;return t||r?`<div class="mc-btvn">${t}${r}</div>`:``}function y(e){let t=e.thanThu;return t?`<aside class="mc-thu">
      ${t.anh===``?`<div class="mc-thu-anh mc-thu-trong" aria-hidden="true"></div>`:`<img class="mc-thu-anh" src="${t.anh}" alt="" />`}
      <div class="mc-thu-chu">
        <div class="mc-thu-ten">${a(t.ten)}</div>
        <div class="mc-thu-cap"><b>Cấp ${t.capDo}/${t.capToiDa??120}</b> · ${a(t.hinhThai)}</div>
        <div class="mc-thu-so">Tháp tầng ${t.tangThapCaoNhat} · ${t.earned===void 0?`thanh tẩy ${t.soCauDaThanhTay} câu`:`${t.earned} EXP đã học`}</div>
      </div>
    </aside>`:``}function b(e,t){return`<header class="mc-em" id="em-${t}">
    <div class="mc-em-trai">
      <div class="mc-em-hang1">
        <span class="mc-ten">${a(e.hoTen||e.sbd)}</span>
        <div class="mc-phu">
          <span class="mc-sbd">${a(e.sbd)}</span>
          <span class="mc-cau-so">Câu ${e.soCau} · Phần ${e.cau.phan}</span>
        </div>
        ${v(e)}
      </div>
      ${e.viSao?`<div class="mc-viSao">${a(e.viSao)}</div>`:``}
    </div>
    ${y(e)}
  </header>`}function x(e){let t=_(e.cau),n=t.replace(/<[^>]*>/g,` `).trim().split(/\s+/).length,r=e.mucDo===`van_dung`||e.sao===2?60:e.mucDo===`hieu`||e.sao===1?30:0,i=/<(?:img|table)\b/i.test(t)?20:0;return Math.max(60,Math.min(180,Math.ceil((40+n*.45+r+i)/15)*15))}function S(e,t,n){if(!e)return`<section class="mc-nua mc-${t} mc-trong" aria-hidden="true"><div class="mc-trong-chu">Đợt này chỉ gọi một em</div></section>`;let r=`giai-${n}-${t}`;return`<section class="mc-nua mc-${t}">
  <button type="button" class="mc-nut-hien-em mc-nut-giai" aria-expanded="false" aria-controls="em-${r}">Hiện học sinh và thần thú →</button>
  ${b(e,r)}
  <div class="mc-than">${_(e.cau)}</div>
  <div class="mc-giai-vung">
    <button type="button" class="mc-nut-giai" aria-expanded="false" aria-controls="${r}">
      <span class="mc-nut-chu">Hiện lời giải</span>
    </button>
    <div class="mc-giai" id="${r}" hidden>${i(e.cau)}</div>
  </div>
  <div class="mc-trang" aria-hidden="true"></div>
</section>`}function C(e,t,n,r){return`<div class="mc-dot" data-dot="${n}" ${r.dayHoc?`data-seconds="${Math.max(x(e),t?x(t):0)}"`:``}>${S(e,`trai`,n)}${S(t,`phai`,n)}</div>`}function w(e,t,n){let r=n.dayHoc?`data-seconds="${x(e)}"`:``,a=`giai-${t}-don`;return`<div class="mc-dot mc-dot-don" data-dot="${t}" ${r}>
  <section class="mc-nua mc-don">
    <button type="button" class="mc-nut-hien-em mc-nut-giai" aria-expanded="false" aria-controls="em-${a}">Hiện học sinh và thần thú →</button>
    ${b(e,a)}
    <div class="mc-than">${_(e.cau)}</div>
    <div class="mc-giai-vung">
      <button type="button" class="mc-nut-giai" aria-expanded="false" aria-controls="${a}">
        <span class="mc-nut-chu">Hiện lời giải</span>
      </button>
      <div class="mc-giai" id="${a}" hidden>${i(e.cau)}</div>
    </div>
    <div class="mc-trang" aria-hidden="true"></div>
  </section>
  <section class="mc-cot-lam-bai" aria-label="Bảng để học sinh lên làm"></section>
</div>`}function T(e,t,r){let i=e.map(e=>`<div class="mc-da-o">
      <span class="mc-da-so">${a(e.phan)}.${e.chuaCho?a(String(e.chuaCho.soCau)):``}${e.chuaCho,``}</span>
      <span class="mc-da-dap">${n(s(e))}</span>
      ${e.chot?`<span class="mc-da-chot">${n(e.chot)}</span>`:``}
    </div>`).join(``);return`<div class="mc-dot mc-dot-da" data-dap-an="1">
  <section class="mc-nua mc-da-trang">
    <header class="mc-em">
      <div class="mc-ten">Đáp án các câu còn lại</div>
      <div class="mc-phu"><span>Cả lớp tự dò · không gọi lên bảng</span><span class="mc-cau-so">${t+1}–${t+e.length} trong ${r} câu</span></div>
    </header>
    <div class="mc-da-luoi">${i}</div>
  </section>
</div>`}var E=`
...
`;function O(e,t={}){let n=t.ngay??new Date,r=[],i=0,o=0;for(;i<e.length;){o++;let n=e[i];if(g(n.cau))r.push(w(n,o,t)),i+=1;else{let a=e[i+1];a&&!g(a.cau)?(r.push(C(n,a,o,t)),i+=2):(r.push(C(n,void 0,o,t)),i+=1)}}let s=t.dsDapAn??[];for(let e=0;e<s.length;e+=12)r.push(T(s.slice(e,e+12),e,s.length));let c=r.length,u=`<div class="mc-thanh">
  <div>
    <div class="mc-thanh-ten">${a(t.tenBuoi||`Gọi lên bảng`)}</div>
    <div class="mc-thanh-phu">${m(n)} · ${e.length} em · ${c} trang${s.length>0?` · ${s.length} câu chỉ đọc đáp án`:``}</div>
  </div>
  <div class="mc-dem" id="mc-dem">Đợt 1/${c}</div>
  <div class="mc-dieu">
...
<div class="mc-ray" id="mc-ray">${r.join(``)}</div>${t.dayHoc?`<div id="mc-clock" role="timer" aria-label="Thời gian làm bài còn lại" hidden></div>`:``}`;return`<!DOCTYPE html>
...
<title>${a(t.tenBuoi||`Gọi lên bảng`)} — tờ máy chiếu</title>
<style>${l}</style><style>${E}</style></head>
<body class="mc${t.dayHoc?` mc-day-hoc`:``}">${u}
<script>${D}<\/script></body></html>`}export{d as SO_DAP_AN_MOI_TRANG,h as chuDeChieu,g as laCauDai,O as taoHtmlMayChieu,x as thoiGianDayHoc};