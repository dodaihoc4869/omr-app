...
chuan(`lần 1 phải xoá đúng 4 bản ghi (1 ca cache + 2 bank + 1 bài làm) và 8 khoá settings (được ${JSON.stringify(lan1)})`, lan1.soBanGhi === 4 && lan1.soKhoaSettings === 8)
chuan(`lần 2 phải đếm 0 (được ${JSON.stringify(lan2)})`, lan2.soBanGhi === 0 && lan2.soKhoaSettings === 0)
chuan('nhóm DỌN phải sạch', sau.bank === 0 && !sau.ca && !sau.luot && !sau.soCau && !sau.khoChua && !sau.deRieng && sau.cheDo === false && sau.qidRaPhieu.length === 0 && sau.diem.length === 0 && !sau.khoDoKho && !sau.onLai)
chuan('nhóm GIỮ phải nguyên', sau.url === 'https://may-chu.example/x' && sau.bime === 'BI-MAT-THU-KHONG-PHAI-THAT' && sau.suaDang.q1 === 'D01' && sau.khoDe.join() === 'D1,D2' && sau.tokHs === 'tok-hs' && sau.tokPh === 'tok-ph' && sau.sbd === '12000' && sau.sdt === '0900000000' && sau.giuPhien === true)
