...
await page.setContent(result.html);await page.locator('.mc-thu-anh').waitFor({state:'attached'});await page.waitForTimeout(300)
assert.equal(await page.locator('.mc-thu-anh').evaluate(img=>img.complete&&img.naturalWidth===360),true)
assert.equal(await page.locator('html').getAttribute('data-projector'),'soft')
...
await page.selectOption('#mc-palette','dark');assert.equal(await page.locator('html').getAttribute('data-projector'),'dark')
await page.selectOption('#mc-size','1.2');await page.getByRole('button',{name:'Hiện lời giải',exact:true}).first().click()
assert.equal(await page.locator('.mc-giai').first().isVisible(),true)
await page.screenshot({path:'/tmp/projector-dark.png',fullPage:true})
