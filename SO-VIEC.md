...
- [x] (Boss soát eb30796, 21/09: "sửa 2 chỗ rồi mới đẩy") (1) `/ca/them-phut` cộng cả lượt `khoa` bằng câu UPDATE riêng (`moKhoaEm` đưa `khoa` → `dang_lam` với nguyên `het_gio_luc`), trả `soLuotKhoaCong`; `soLuotCong` vẫn chỉ đếm đang làm; (2) `publish` (`dayCa`) và `/ca/nhieu` không hạ `thoi_gian_phut` của ca đã thêm phút (ghi MAX(số app gửi, hiện có); đọc bằng `docPhutCaDaThem` nên chưa chạy migration vẫn ghi như cũ) | bằng chứng: `tests/them-phut-may-chu-1909.test.ts` 14 test xanh (em mở khoá sau khi thêm phút nhận hạn mới; publish giữ 55 khi app gửi 45, theo 60 khi gửi 60, ca chưa thêm hạ được; /ca/nhieu; thiếu cột vẫn ghi); đột biến 9/9 bị bắt; toàn vitest 97 đỏ/42 tệp = nền đúng
- [ ] (Boss 21/09, thầy chốt BTVN "nâng đỡ", ĐỔI LUẬT CHẤM: điểm trên số câu của từng em; việc SỐ 1) Máy chủ: migration chỉ-thêm (`btvn.ca_nhan/hat_giong/so_loi`, `btvn_cau`, `btvn_em_cau`, cột `btvn_em`), `/btvn/giao` nhận `caNhan/cau[]/ghim[]/hatGiong`, `btvnCuaEm` chốt + trả bộ theo chặng, chấm theo câu của em, `/btvn/xong-lo` chặng ≡ lô, `/btvn/xem-truoc` chỉ đọc, `theoDoiBtvn` + chống chép trên lõi, kế hoạch ngày, hồ sơ lên bảng; bài `ca_nhan=0` y cũ; cờ `cau_hinh.btvn_ca_nhan` | bằng chứng: BƯỚC 1 XONG: hợp đồng `docs/hop-dong-btvn-nang-do-2109.md` (đã dò đường BTVN hiện tại, ghi rõ: `/btvn/cua-em` hiện trả `dap_an` trong `de.cau`); còn: migration, cài đặt, test, đột biến, báo Boss
- [x] (Code 3 21/09, việc SỐ 1 Boss: BTVN "nâng đỡ" máy chủ ĐỢT 1; `docs/hop-dong-btvn-nang-do-2109.md` bản 4) Đã viết trong cây làm việc, CHƯA đẩy (chờ Boss soát): migration `server/migration-2109-btvn-nang-do.sql` (chỉ-thêm, CHƯA chạy --remote); `server/src/btvn-nang-do-d1.ts` + `btvn-nang-do-chang.ts`; nối `index.ts` (giao `caNhan`/`cau[]`/`ghim[]`/`hatGiong` + cờ `cau_hinh.btvn_ca_nhan`, `/btvn/cua-em` chốt bộ một lần KHÔNG đáp án, `/btvn/xong-lo` = nộp chặng, `/btvn/xem-truoc` chỉ đọc, `/btvn/theo-doi` + `/btvn/bai-lam`), `goi-cu.ts` (`nopBtvnQuaPhieu` luật điểm + thưởng, `hsBtvn`, `hoSoLopLenBang` chỉ câu của em), `ke-hoach-ngay*.ts` (lô ≡ chặng), `reset-toan-app.ts` (`btvn_cau`, `btvn_em_cau` vào BANG_XOA), `su-kien-hoc.ts` (xuất `chuyenDeThat`) | bằng chứng: 61 test mới xanh (`tests/btvn-nang-do-may-chu-2109` 43 · `-xem-truoc-` 8 · `-thay-may-chu-` 10) + 21 đột biến đều bị bắt; đo D1: mở lần đầu 8 truy vấn, mở lại 3, xem trước 4 (a) / 6 (b); toàn bộ vitest 98 đỏ / 43 tệp so nền 97 / 42 — đỏ mới DUY NHẤT `tests/bang-nhiem-vu-1909.test.tsx` (`src/components/bang-nhiem-vu/TheCuoiChang.tsx` import react-dom, tệp của Code 2, ngoài làn máy chủ); mọi test cũ khác giữ nguyên tên đỏ (đối chiếu bằng worktree HEAD sạch)
  · test cũ đổi: KHÔNG có (chỉ thêm test mới + `tests/_btvn-nang-do-mau.ts`)
  · giới hạn đã biết: `suaBtvn reset` bài cá nhân hoá xoá đáp án/tiến độ nhưng giữ bộ đã chốt; sổ `su_kien_hoc` giữ kết quả lần đầu (INSERT OR IGNORE) nên kết quả làm lại sau reset không ghi đè sổ
- [x] (Boss 21/09, thầy chốt "Cho làm lại") Worker Đợt 1 ĐÃ LÊN: migration `migration-2109-btvn-nang-do.sql` chạy `--remote` 06:5x (3 cột `btvn`, 6 cột `btvn_em`, `btvn_cau`, `btvn_em_cau`), Worker `11e83882-b880-4159-8586-9534ff43a9e5` (06:53, `./DAY-MAY-CHU.command`); thử thật `/btvn/xem-truoc` không ghi: tờ DB-12-B6-D3 48 câu, 3 SBD ⇒ 24/20/20 câu, lõi 14, 7 chặng; quy ước qid khớp trên 3 tờ thật (boQuaQid [] · thieuMeta 0; chỉ câu III có đáp án dài bị bỏ)
- [x] (Boss 21/09) `/btvn/cho-lam-lai {maBtvn, sbd}` (bài `ca_nhan` đã nộp, còn hạn; chép `btvn_em_lich_su`, `so_lan_lam+1`, xoá nộp/đáp án/điểm, `lo_da_xong=0`, GIỮ bộ câu + hạn) + `btvn_lo.lan = chiSo + 1000×(lượt−1)` + `exp-d1.ts` lấy `lan % 1000` làm chỉ số lô (EXP không cộng đôi) + `/btvn/xem-truoc` (a) trả `boQuaQid`/`thieuMeta` + tự nộp cuối tính EXP sau khi nộp + `ai_ho_so_ngay/ai_dieu_chinh/ai_ban_tin` vào BANG_GIU của reset | bằng chứng: `tests/btvn-nang-do-cho-lam-lai-2109.test.ts` 8 test, đột biến 10/10 bị bắt; các test cũ liên quan xanh; CHƯA đẩy
- [x] (Boss 21/09, Code 1 c5153a5 đã soát) NỐI 6 route `/ai/cau-hinh`, `/ai/ho-so-ngay`, `/ai/dieu-chinh/nop`, `/ai/dem-qua`, `/ai/nhat-ky`, `/ai/dieu-chinh/bo` vào `server/src/index.ts` SAU cổng `laThay` (hàm của `server/src/bo-nao.ts`, tự trả {ok,…}) | bằng chứng: `tests/bo-nao-noi-route-2109.test.ts` 4 test (không mã ⇒ 403 + không ghi; có mã chạy; nằm sau cổng theo mã nguồn; điều chỉnh "áp dụng" trong bảng KHÔNG đổi byte `/hs/ke-hoach-ngay`), đột biến (route đặt trước cổng) bị bắt; 81 test bo-nao + reset xanh; CHƯA chạy migration/đẩy (chờ Boss soát commit nối)
  · test cũ đổi: `tests/bo-nao-may-chu-2109.test.ts` (Code 1) dòng cuối "khoá nguồn": từ `not.toContain('bo-nao')` (index.ts chưa nối) thành `toContain("from './bo-nao'")`; lý do: index.ts nay ĐÃ nối bộ não; bỏ cụm "không sửa index.ts" khỏi tên test
- [x] (Boss 21/09) Worker lô A `11d7be41` (07:07, 2de35c7: `/btvn/cho-lam-lai` + lan sổ + xem-truoc boQuaQid/thieuMeta; thử thật: không mã ⇒ 403, bài không tồn tại ⇒ lời thật) và Worker `23ba8b32-6c00-41b1-b74a-bf29a61234ca` (07:12, 2d05cb1: 6 route `/ai/*` sau `laThay`; migration `migration-2109-bo-nao.sql` chạy `--remote` trước, 3 bảng `ai_*`) ĐÃ LÊN | bằng chứng: thử thật `/ai/cau-hinh` đọc ⇒ `{bat:true, cheDo:'bong', lopThat:[]}`; `/ai/ho-so-ngay` trang 1 ⇒ 40 em/trang, 267 em, 7 trang, luồng sâu 18 · vắng 11 · bỏ qua 11 · nhanh 0, 806 ms; chế độ BÓNG (chưa tầng nào đọc `ai_*`); lùi Worker: `cd server && npx wrangler rollback` về `11d7be41`
- [ ] (Boss 21/09, thầy chốt bản 1.1) LỊCH CHẶNG THEO GIỜ: dùng `xepLichChang`/`sucChua` của Code 1 (CHƯA có trong repo) thay `moLucChang`, LƯU vào `btvn_em.chang_mo_json` lúc chốt (bài đã chốt không JSON ⇒ cách cũ), trả `moLuc` từng chặng, `chiTiet.lich` + `chiTiet.theoGio` + `canhBaoHanNgan` ở xem-truoc (dạng trường đã chốt với Code 4), `nganSachChoEm` dùng sức chứa theo phiên, kế hoạch ngày đọc lịch đã lưu | bằng chứng: (chưa có)
- [ ] (Boss 21/09) CHẾ ĐỘ `that` của Bộ não: kế hoạch ngày + BTVN đọc `ai_dieu_chinh` (`ap_dung=1`, `huy=0`, `het_han>=hôm nay`) qua cổng `dieuChinh`; `on_som` chỉ kéo `moc_on_ke` SỚM hơn; `/hs/ke-hoach-ngay` trả `loiNhanHlv`; `/ph/*` trả `boNaoAi {loiNhan, thuTuan, ngay}` cho đúng con; chạy thử ⇒ KHÔNG trả gì; không núm nào đổi `han_nop` (test khoá); bảng tin thầy nhận mục "Bộ não đêm qua" | bằng chứng: (chưa có)
- [x] (Boss 21/09, thầy chốt Bộ não CHẠY THẬT) TẦNG ĐỌC điều chỉnh: `server/src/bo-nao-doc.ts` + nối `ke-hoach-ngay-d1.ts` (nhịp + on_som), `ke-hoach-ngay.ts` (`tinhNganSach` nhận `boNao.nhip`), `btvn-nang-do-d1.ts` (chốt qua cổng `dieuChinh` + `thichNghiSauChang` sau mỗi chặng, kiểm lại bất biến), `index.ts` (`loiNhanHlv` ở `/hs/ke-hoach-ngay`), `ph-truy-cap.ts` (`boNaoAi` ở `/ph/ke-hoach`) | bằng chứng: `tests/bo-nao-doc-2109.test.ts` 16 test (bóng/tắt/hết hạn/huỷ/tự gỡ/tin cậy thấp ⇒ y hệt từng byte; nhịp; on_som chỉ sớm; lời nhắn HS/PH đúng con; thích nghi giữ chặng đã mở/lõi/hạn; công tắc dừng ngay), đột biến 20/21 (còn 1 cổng tương đương); 21 tệp liên quan 417 test xanh; CHƯA đẩy; docs/bo-nao-tang-doc-2109.md
- [x] (Boss 21/09, thầy chốt bản 1.1) LỊCH CHẶNG THEO GIỜ (worktree `btvn-lich-gio`, CHƯA vào main/CHƯA đẩy): `xepLichChang`/`sucChua` của Code 1 thay `moLucChang` ở chốt; lưu `btvn_em.chang_mo_json`; mở bài/nộp chặng/thích nghi/xem-truoc/kế hoạch ngày đọc lịch đã lưu (bài chốt trước 1.1 ⇒ lịch cũ); ngân sách hạn ngắn theo sức chứa; xem-truoc `chiTiet.lich`+`theoGio`+`canhBaoHanNgan`; `/btvn/cua-em` thêm `theoGio` + `chang[].dungNhipTruoc` | bằng chứng: `tests/btvn-nang-do-lich-gio-2109.test.ts` 10 test + đột biến 11/11; 8 tệp liên quan 189 test xanh; docs/hop-dong-btvn-nang-do-2109.md mục 10
  · test cũ đổi (bản 1.1): `tests/btvn-nang-do-xem-truoc-2109.test.ts` `soNgay: 7` → `8` (số ngày lịch VN của Code 1) và tờ 60 câu hạn 2 ngày → 3 ngày (hạn 2 ngày nay là hạn NGẮN, ngân sách theo phiên); `tests/btvn-nang-do-cho-lam-lai-2109.test.ts` test EXP: thêm `chang_mo_json = NULL` khi lùi `chot_luc` (bài chốt trước 1.1)
- [x] (Boss 21/09) THÍCH NGHI sau mỗi chặng BẬT cho MỌI em bài cá nhân hoá, commit RIÊNG, cờ tắt `cau_hinh.btvn_ca_nhan.thichNghi = false` (worktree `thich-nghi-co`, dựa trên bản 1.1; CHƯA vào main/CHƯA đẩy) | bằng chứng: `tests/btvn-nang-do-thich-nghi-2109.test.ts` 10 test (cờ; bật mặc định lưu thay đổi hợp lệ; cờ tắt ⇒ lõi không được gọi; lõi vi phạm bất biến ⇒ bỏ; tất định) + đột biến 4/4; 8 tệp liên quan 150 test xanh; docs/hop-dong-btvn-nang-do-2109.md mục 11
  · giới hạn đã biết: ngưỡng mẫu của lõi (≥ 2/3 câu thường cùng dạng ở một chặng) khiến bộ nhỏ/chặng nhỏ ít khi có thay đổi tự nhiên — test ép lõi trả thay đổi hợp lệ để khoá phần nối; đo thật sau khi có em làm nhiều chặng
- [x] (Boss 21/09, thầy lệnh "tuyệt đối không rút câu tự luận, chỉ rút trắc nghiệm / đúng sai / trả lời ngắn, TRỪ Gọi lên bảng"; `prompt-cam-rut-tu-luan.md`) KIỂM KÊ + LỌC MỌI KÊNH RÚT CÂU Ở MÁY CHỦ | bằng chứng: `tests/cam-tu-luan-may-chu-2109.test.ts` (32 ca, kho mẫu 4 câu rút được + 4 tự luận gồm câu saccharose của thầy; mỗi kênh: 0 tự luận ra + đối chứng câu rút được VẪN ra) + 22 đột biến (bỏ từng dòng lọc) đều bị test bắt
  · MỘT định nghĩa: `src/lib/cau-tu-luan.ts` (Code 1: `laCauTuLuan`, `locCauRutDuoc`, `laMaDeTuLuan`); lớp mỏng máy chủ `server/src/cam-tu-luan.ts` (đọc JSON câu, gỡ tự luận khỏi gói, lọc phiếu, đệm qid tự luận theo phiên bản tờ). Đã chạy thử trên 4 tờ kho THẬT (DB-12-B6-D3, DB-12-B5-D1, DB-11-B8-D3, DH-12-C2-B6; 299 câu): chỉ 6 câu bị coi tự luận (đáp án chữ dài / nhiều ý), luật cũ của BTVN bỏ ĐÚNG cùng những câu ấy.
  · KÊNH → ĐÃ LỌC (mã dòng) / cách lọc:
    – game Đảo `start`/`recommendations`: `game-v2.ts:122` (`eligible` bỏ `laCauTuLuan`); Linh Tâm + Võ đài + Đoàn `taoNguoi` đều đi qua `start` nên hưởng luôn
    – game Đảo `so-tay`: `game-v2.ts:86`
    – game Đảo lượt ĐÃ TẠO trước lệnh cấm: `currentQuestion` `game-v2.ts:57` (answer câu tự luận ⇒ "câu đã rút khỏi kho"); `resume` bỏ câu tự luận khỏi lượt trả về, lượt toàn tự luận ⇒ `questions: []` (`game-v2.ts` `qidTuLuanTrongLuot` :65); `complete` chỉ đòi câu rút được (Code 2 báo em kẹt cuối lượt)
    – Đoàn Hộ Tống: câu cá nhân qua `start` (trên); câu chung của trùm `chonCauTrum` `game-v2-doan.ts:207`; phòng cũ đã ghim câu tự luận `cauRieng` `game-v2-doan.ts:135` ⇒ null ⇒ đường `rut: true` sẵn có
    – ôn lại: `qidPhucVuDuoc` `cau-theo-qid.ts:58` (một định nghĩa "phục vụ được" cho kế hoạch ngày + lệnh lấy đề; thêm `tuLuan[]`), `layCauChoEm` `cau-theo-qid.ts:129` (cũng là cửa của `/hs/on-lai/nop`); lệnh đo `docDoPhuPhucVu` thêm `qidTuLuan`
    – kế hoạch ngày (on_lai/on_thi/keoOnSom): hưởng `qidPhucVuDuoc` (hàm thuần `ke-hoach-ngay.ts` chỉ nhận qid, không sửa); `soCauToiHan` chỉ đếm câu em làm được; câu tự luận em từng sai VẪN ở `nam_kt_cau`
    – Mẹ giao `mom create`: `mom.ts:43` (máy phụ huynh tự chọn câu rồi gửi lên ⇒ máy chủ là lớp chặn cuối; trả `soBoTuLuan`; bài toàn tự luận bị từ chối); bài đã lưu trước đây KHÔNG sửa (để thầy quyết: đổi `question_count` lệch mã `cau_N`)
    – bài hằng ngày cho phụ huynh: `dungDuoc` `parent-news-nguon-cau.ts:61` (đứng trước luật 3 dạng chuẩn cũ, chặt hơn ở phần III, giữ nguyên)
    – luyện đề 2026: `luyen-de.ts:60`
    – khắc phục sau ca: `goiTheoDang` (`dsDang`) `goi-cu.ts:2284`; nhánh theo chuyên đề đọc gói TRƯỚC khi chốt `thuTu` và gỡ tự luận khỏi gói trả về `goi-cu.ts:2427`; `hsCauSai`/`hsCauDaThi` `goi-cu.ts:3340` (cộng thêm luật 3 dạng chuẩn cũ); tờ dạng bài `deTheoDangBai` `goi-cu.ts:3630`
    – phiếu bài tập: `luuPhieu` `goi-cu.ts:502` (chỉ `loai='baitap'`; phiếu kết quả là báo cáo, không đụng), `layPhieu` `goi-cu.ts:553` (lọc lúc phục vụ cho phiếu ĐÃ LƯU từ trước), `ghiPhieuKhacPhuc` `goi-cu.ts:2455` (toàn tự luận ⇒ không lập phiếu)
    – `/kho/rut-cau` (thầy; chỉ mục không đáp án): `index.ts:2233` đọc gói từng tờ (đệm theo phiên bản tờ) và bỏ qid tự luận; không đọc được gói ⇒ giữ (không kết tội); trả `coSan` + `soBoTuLuan`
    – BTVN thường + nâng đỡ: cửa DUY NHẤT `homeworkQuestions` `btvn-grading.ts:117,129` (thay luật cũ phần III đáp án dài bằng hàm chung + tờ `-TL`); `giaoBtvn` đếm `soCau` = số câu em sẽ được giao (trước đếm cả tờ) + trả `soBoTuLuan`/`chuBoTuLuan` `index.ts:1515`; nâng đỡ `chuanHoaCauGiao` hưởng qua `homeworkQuestions`, qid tự luận thầy gửi vào rơi vào `boQuaQid`
  · KHÔNG SỬA (theo lệnh thầy / không phải rút): Gọi lên bảng (`ho-so-len-bang.ts` + màn chiếu; test khoá: tệp không nhập bộ lọc, `/kho/lay` trả NGUYÊN tờ gồm câu tự luận), ca thi + bài thầy tự chọn bộ câu (`choThiLai`, `noiKhoCa`, `layDeKho`), chấm điểm/sổ (`cauTuKhoTheoQid`, `dapAnTheoMaDe`), bảng `cau_hoi` (giữ nguyên dòng chỉ mục: hồ sơ `ho-so-nam-kt.ts` tra chuyên đề theo qid), `readScope` (pool còn nuôi bằng chứng của hồ sơ; chỉ nơi CHỌN mới lọc), `dayDeKho`/`dungChiMucKho`/`syncIndex` (không lọc lúc dựng chỉ mục: tờ đã lập chỉ mục không tự dựng lại, lọc lúc ĐỌC thì đúng ngay cả với chỉ mục cũ)
  · test cũ đổi (3, CÓ CHỦ Ý, ý nghĩa giữ nguyên): `tests/cau-theo-qid-1909.test.ts` (fixture câu phần I để đáp án bí mật dạng chuỗi ⇒ nay là tự luận; thêm `correct: 'B'`, mọi ràng buộc "không lộ đáp án" giữ), `tests/khoi-a-va-kho-de-1209.test.ts` (khoá nguồn `rutCau`: `coSan: con.length` → `coSan: con0.length - soBoTuLuan`, cửa sổ đọc 1600 → 3600 ký tự) và `tests/man-phan-cong-btvn-1209.test.ts` (khoá nguồn `giaoBtvn`: đếm theo phần vẫn `cau.filter(phan)` nhưng qua `locCauRutDuoc(`). Hành vi đổi có chủ ý: (1) `soCau` của `/btvn/giao` nay đếm câu rút được (không đếm tờ `-VD/-DT/-TL` và câu tự luận); (2) `phần III không đáp án` không còn phát cho em ở BTVN thường (trước phát mà không chấm)
  · giới hạn đã biết: `/kho/rut-cau` đọc R2 lần đầu sau mỗi lần khởi động isolate (đệm sau đó); `coSan` có thể lệch CAO vài câu (chỉ duyệt tới khi đủ `soCau`); máy thầy vẫn phải lọc phía Code 1 (`ho-so-on-ca` chỉ trả qid)
- [x] (Boss 21/09, thầy "xong 1.2 rồi báo tôi mới giao BTVN") Worker `e5ce749b-6331-4f1f-82a7-f4e8f61a06aa` 08:41 (bản 6d849c6: cấm tự luận máy chủ + 3af2d02 bộ não); thử thật `/ai/ho-so-ngay` 21/09 sau chốt cả lớp: 220 em có thẻ → sâu 55 (= trần 25 %), nhanh 74, vắng 91, `dangCaLopYeu` = 1 dạng (44 em)
- [x] (Boss 21/09, việc số 1; quyết định "chặng ảo" của Boss) NỐI BẢN 1.2 LÕI BTVN (Code 1, 3eb05bc) — nhóm "THỬ SỨC THÊM · không bắt buộc" (`thuSucThem`) | bằng chứng: `tests/btvn-nang-do-12-may-chu-2109.test.ts` (22 ca) + 20 đột biến đều bị bắt; vitest toàn bộ đỏ mới = [] (97 test / 42 tệp = nền), `tsc -b` + `tsc -p server` sạch
  · LƯU: `btvn_em_cau.chang = -1`, nhãn `loi_cao` (KHÔNG migration: thêm giá trị mới cho cột sẵn có, hằng `CHANG_THU_SUC_THEM`); `so_cau_em`/`so_chang`/`tom_tat_json` CHỈ đếm câu bắt buộc; mọi truy vấn chặng dùng `chang >= 0` (`docBoCuaCacEm`, `thongKeTheoDoiCaNhan`, kế hoạch ngày `ke-hoach-ngay-d1.ts` lô ≡ chặng, thích nghi DELETE)
  · MỞ BÀI (`/btvn/cua-em`): `soThuSucThem` (số, luôn có) + khoá `thuSucThem: {chiSo: soChang, moLuc (= mốc mở chặng cuối), cau[] (đã bóc đáp án; RỖNG cho tới moLuc), daNop (mọi câu đã có đáp án)}` CHỈ khi bài có thử sức (vắng ở bài cũ / em khá); `de.cau`, `nhan`, `soCau`, `soCauCuaEm`, `soChang` KHÔNG gồm nhóm này
  · NỘP RIÊNG: `POST /btvn/xong-lo` với `chiSo = soChang` (chặng ẢO — không tính `so_chang`/`lo_da_xong`/xong bài), từ `moLuc` tới `han_nop` KỂ CẢ khi phần bắt buộc đã tự nộp (`qua_han`/`chang_chua_mo` như chặng thường); đáp án đầu khoá, idempotent (CAS trên `dap_an_json`, mỗi câu cộng đúng MỘT lần); `ketQua[]` như chặng thường (đúng/sai + lời giải); phản hồi thêm `thuSucThem {chiSo, soCau, soDaLam, soDung, daNop}` và `baiDaNop {soDung, soCau}` khi bài đã nộp
  · ĐIỂM (luật câu THƯỞNG): thử sức ĐÚNG ⇒ +1 tử số +1 mẫu số (bài đã nộp: cập nhật NGAY `so_dung`/`so_cau`; bài chưa nộp: lưu đáp án, tính lúc nộp cuối); SAI/bỏ trống ⇒ không đổi; sổ `su_kien_hoc` CHỈ ghi câu ĐÚNG (không hẹn ôn, không thành câu sai của hồ sơ); EXP theo câu như thường, không EXP "xong chặng"; `/btvn/nop` không nhận đáp án thử sức trong thân lệnh (chỉ gộp đáp án đã khoá)
  · THẦY: `/btvn/xem-truoc` `chiTiet.thuSucThem` (+ `tomTat.soBatBuoc/soThuSucThem` của lõi); `/btvn/bai-lam` `thuSucThem {soCau, daLam[{qid, dapAnEm, dung}]}`; `/btvn/theo-doi` `soCauLoi`/`diemLoi` theo lõi BẮT BUỘC của em, `soCauThuongSai` không tính thử sức; `/hs/btvn` `soThuSucThem`
  · bài ĐÃ CHỐT giữ nguyên (không dòng chang = -1 ⇒ `soThuSucThem: 0`, không khoá `thuSucThem`, nộp `chiSo = soChang` ⇒ `chang_chua_mo`); thích nghi không đụng nhóm (test ép lõi trả thay đổi thật)
\n- [x] (Boss 21/09, thầy "cảnh báo TỰ ĐỘNG gửi luôn, chỉ báo lên cho tôi") HÔM NAY v2 · MÁY CHỦ: cảnh báo tay + NHẮC TỰ ĐỘNG + 5 lệnh đọc của thầy | bằng chứng: `tests/canh-bao-thay-server-2109.test.ts` (19), `tests/nhac-tu-dong-2109.test.ts` (31), `tests/gv-hom-nay-v2-server-2109.test.ts` (20) + đột biến; hợp đồng `docs/hop-dong-hom-nay-v2-2109.md` (mục 9 nhắc tự động)
  · migration chỉ-thêm `server/migration-2109-canh-bao-thay.sql` (bảng `canh_bao_thay`: id, ma_btvn, sbd, ngay, ten_btvn, han_nop, loi_em, loi_ph, trang_thai_em, gui_luc, em_xem_luc, ph_xem_luc, moc, gui_ph, ph_nhom) — CHƯA chạy `--remote` (chờ Boss soát); chưa chạy thì lệnh cảnh báo trả lỗi bằng lời, nhắc tự động chạy vô hại (bảng chưa có ⇒ lỗi được nuốt), mọi lệnh khác như cũ
  · GHI: `/gv/canh-bao-nop-bai` (thầy bấm tay, ≤ 60 em, 1 tin tay/em/bài/ngày) + `nhacTuDong` (cron `* * * * *` ⇒ chiếm lượt 30 phút bằng `cau_hinh.nhac_tu_dong_lan`, khung 07:00–21:30 VN, 4 mốc M1–M4, trần, gộp phụ huynh, cờ `cau_hinh.canh_bao_tu_dong` mặc định BẬT); em: khoá `canhBaoThay` ở `/hs/ke-hoach-ngay` + `/hs/canh-bao/xem`; phụ huynh: `/ph/ke-hoach` + `/ph/canh-bao/xem` (xem một dòng ⇒ cả nhóm gộp đã xem); push qua `student_notice` (target `canh_bao_btvn`, `deliverNotices` mở rộng)
  · ĐỌC (đo ≤ 12 truy vấn, KHÔNG ghi): `/gv/tim-em`, `/gv/chua-nop` (+ `tuDong`/`homNay`/`daNhac`/`nhacKe`), `/gv/can-giup`, `/gv/vinh-danh-ngay`, `/gv/em-toan-canh` (dòng thời gian ≤ 30/trang, `mo_app` chưa có nguồn nên KHÔNG trả); `/gv/nhac-tu-dong` (đọc/ghi cờ)
  · `reset-toan-app.ts`: `canh_bao_thay` vào BANG_XOA; test cũ đổi CÓ CHỦ Ý: `tests/canh-bao-thay-server-2109.test.ts` (chỉ MỘT nơi ghi ⇒ HAI nơi: tay + tự động; bộ não vẫn không có đường ghi)
  · trần chỉ ghi nhận sai lệch nhỏ với đề: em ≤ 1 tin/bài/ngày áp cho M1/M3 (kể cả tin tay); M2/M4 là mốc chốt hạn riêng (ngày hạn có thể M1 sáng + M2 tối) — Boss soát

- [x] (Boss 21/09 sáng, thầy lệnh) GÓI WORKER + DỮ LIỆU THẬT ĐÃ CHẠY 10:39–10:42: sao lưu D1 `/Volumes/SSD NGOÀI/omr-saoluu/omr-d1-20260921-1039-truoc-goi-worker.sql` → migration `canh_bao_thay` + `ten_lop` → gán Tinh Hoa 42 → Worker a19afa84 (gói 3230449) → gán Nhóm 10 điểm 15 → đổi tên 12028 "Nguyễn Công Huy" (chỉ có dòng ở danh_sach + hoc_sinh) → migration bảng lưu `hoc_sinh_da_go`/`danh_sach_da_go` → gỡ 12121 (hồ sơ + danh sách) + 12126 (chỉ danh sách) → Worker **f9c4e6b6** (main a3e6235) ĐANG SỐNG, bản lùi e268ce35 | bằng chứng: câu kiểm từng bước (42 · 15 · Huy · bảng lưu 1+2, còn 0); kiểm sống `/gv/lop` 3 lớp khối 12 (Thường 67 · Nhóm 10 điểm 15 · Tinh Hoa 42), `/em/danh-sach` tenLop 265/265, `/gv/buoi-chua-de-xuat` ok 11 truy vấn. Thứ tự + lệnh + lùi: `docs/chay-that-2109-thu-tu.md`.
- [x] M2 nhắc tự động = BUỔI TỐI CUỐI TRƯỚC HẠN (hạn ≥ 20:30 ⇒ 20:00 ngày hạn; hạn sớm hơn, vd 12:00 trưa ⇒ 20:00 HÔM TRƯỚC; lời "Hạn nộp 12:00 trưa mai (Thứ Bảy 26/09)"; chữ "còn N trong K chặng") | 35d9b94, `tests/nhac-tu-dong-2109.test.ts` 36 test, đột biến 9/9.
- [x] Nối `lichDaiSomHon` (Code 1, bản 1.3) ở MỌI nơi máy chủ đọc `chang_mo_json` (`docLichDaLuu(v, soChang, nguCanh?)`: mở bài, nộp chặng, thử sức thêm, xem trước, thích nghi, kế hoạch ngày, nhắc tự động) — chỉ sớm hơn, không migration | cedb77a, `tests/btvn-nang-do-lich-13-may-chu-2109.test.ts` 6 test. Bản 1.4 (trần ngân sách hạn ngắn, `nganSachHanNgan` + số câu lõi) ở `nganSachVaSucChua` | 9ede01d, `tests/btvn-nang-do-ngan-sach-14-may-chu-2109.test.ts` (bài 113 câu hạn 24/09 12:00: 36 · 24 · 12).
- [x] TÊN LỚP: cột `hoc_sinh.ten_lop` (lop giữ = KHỐI), `/gv/lop`, `/gv/doi-lop-em`, `tenLop` ở `/em/danh-sach` + `listStudents`; mặc định theo khối (khối 12 ⇒ "12 - Lớp Thường") | 98d26c2 + 3230449 (hợp đồng `docs/hop-dong-ten-lop-2109.md`), `tests/ten-lop-may-chu-2109.test.ts` 20 test. `/gv/buoi-chua-de-xuat` (B6, chỉ đọc ≤ 12 truy vấn; hợp đồng của Code 1) | e429cbf, `tests/buoi-chua-de-xuat-may-chu-2109.test.ts` 14 test, đột biến 25/25.
- [x] Tệp SQL chạy thật MỘT lần (ngoài danh sách migration; mỗi tệp có lệnh lùi + câu kiểm): `server/gan-2109-ten-lop-tinh-hoa.sql`, `gan-2109-ten-lop-nhom-10-diem.sql`, `doi-ten-2109-12028(-lui).sql`, `migration-2109-hoc-sinh-da-go.sql` (bảng lưu, GIỮ khi reset), `go-2109-em-chua-mo-app(-lui).sql` (lưu rồi mới xoá, tự giữ em có dấu vết) | a3e6235. Danh sách em khối 12 chưa mở app (lập từ sao lưu 02:25 + đối chiếu D1 sống của Boss): tệp ngoài git `/Volumes/SSD NGOÀI/omr-saoluu/khoi12-chua-mo-app-20260921.tsv` (nay còn 2 em; bản 15 em giữ riêng).
- [!] Bẫy đã gặp: (1) công cụ `Write`/lệnh đổi chuỗi thoát unicode NUL gõ trong mã thành KÝ TỰ THẬT — sau khi ghi tệp có chuỗi thoát phải quét byte NUL; (2) phiên khác `git stash`/`pop` trong cây chính có thể hoàn nguyên rồi trộn lại sửa chưa commit (index.ts nhân đôi dòng) — sửa xong commit NGAY theo đường dẫn; (3) hộp phân loại quyền chặn `wrangler ... --remote` khi chỉ có tin của phiên khác; sau lời thầy trong khung chat ("tuân theo điều phối 100%") thì chạy được.
