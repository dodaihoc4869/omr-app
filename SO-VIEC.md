...
- [!] CẢNH BÁO SỐ LIỆU THẬT sau việc nhỏ 1 (báo 0.Planer, KHÔNG phải lỗi mã): D1 thật có 42 bài Mom TAY chưa bắt đầu của 5 em (tổng 439 câu): 11036 = 2 bài / 229 câu, 11037 = 24 bài / 137 câu, 10006 = 9 bài / 43 câu, 11010 = 5 bài / 20 câu, 11074 = 2 bài / 10 câu, cũ nhất 16/09. Nay tất cả đều là việc BẮT BUỘC trong viec[] nên 5 em này sẽ bị nhãn `qua_tai` (tải cứng vượt mục tiêu 8–16) và em 11037 có ≥ 24 dòng `mom`. Đúng lệnh ('tính vào tai.cung'), giao diện cần vẽ được danh sách dài / gộp; nếu muốn giới hạn (vd chỉ tính bài giao ≤ N ngày, hoặc gộp) là quyết định của 0.Planer — tôi không tự đặt trần.
- [ ] (0.Planer, việc nhỏ 2) LỆNH MỚI `POST /hs/cau-theo-qid {token|sbd, qid[≤20]}`: nội dung câu để em LÀM đúng các câu việc on_lai/on_thi đã chọn; KHÔNG kèm đáp án/lời giải; chỉ qid thuộc kế hoạch hôm nay của em hoặc đã từng gặp trong `su_kien_hoc`; ≤ 3 truy vấn + R2 theo tờ  | bằng chứng: (chưa có)
