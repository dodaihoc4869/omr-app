...
## Soát adapter Bộ não với server THẬT (adc85f1) + lịch chặng theo giờ (5cfb94a)
- [x] Đối chiếu `server/src/bo-nao.ts` (c5153a5): tên trường khớp (dem-qua, nhat-ky, dieu-chinh/bo, cau-hinh). Sửa cho ĐÚNG hành vi: (1) chỉ "ĐÃ LÀM" khi `apDung:true`, chỉ "đã hỗ trợ N em" khi `soEmHoTro` do máy chủ nói (không suy từ số điều chỉnh qua kiểm khuôn) — thầy chốt chạy thật mà tầng áp dụng chưa bật thì màn nói "chưa áp"; (2) `/ai/dieu-chinh/bo` trả `daBo:false` (không có gì để bỏ) ⇒ không nói "đã bỏ"; (3) `soEm=0` ⇒ "chưa soi em nào"; (4) nhịp mặc định {lech 0, khoiDong 2} không in như một thay đổi; (5) lời xác nhận chuyển lớp sang Thật nói đúng ("có thể mới chỉ ghi nhận").
- [x] Lịch chặng theo GIỜ ở Xem trước (dạng trường Code 3 chốt: `chiTiet.lich[{chiSo,moLuc,soCau}]` + `chiTiet.theoGio`, `canhBaoHanNgan{soCauLoiToiThieu,soPhien}` ở gốc): "Chặng 1 · 20:00 · 8 câu", chỉ hiện giờ khi `theoGio:true`; cảnh báo "Hạn ngắn: mỗi em tối thiểu N câu lõi trong M phiên — cân nhắc lùi hạn." đứng đầu dải cảnh báo. Máy chủ chưa trả các trường này (chờ xepLichChang của Code 1) ⇒ màn như cũ. Test + đột biến 5/5, ảnh `bn-theo-gio-*.jpg`.
- Lưu ý: tệp `src/lib/bo-nao-hien-thi.ts` (Code 2, chưa commit) từng làm `tsc -b` đỏ vài phút — không phải của phiên này; đã hết.

## Ô NGÀY/THÁNG/NĂM + GIỜ:PHÚT 24 giờ dùng chung — `ONgayGio24` (c4f9a9a, thầy yêu cầu 21/09)
- [x] Lõi thuần `src/lib/ngay-gio-24.ts` + ô `src/components/ONgayGio24.tsx` (+ `o-ngay-gio-24.css`): ba ô số ngày/tháng/năm + giờ 00–23 : phút 00–59 (`inputMode=numeric`), mũi tên lên/xuống (vòng theo số ngày của THÁNG, nhuận đúng), tự nhảy ô, nút nhanh "Hôm nay 23:59 · Mai 23:59 · +3 ngày · +7 ngày", báo "ngày không có thật" / "Hạn nộp phải ở sau thời điểm hiện tại" (chỉ khi thầy VỪA chỉnh — gia hạn bài quá hạn không đỏ từ đầu). Giá trị ra GIỮ dạng cũ 'YYYY-MM-DDTHH:mm' / 'YYYY-MM-DD'; nhập dở/không có thật ⇒ '' như ô cũ để trống.
- [x] Thay ở: PhanCongScreen (hạn nộp mới — Giao BỊ CHẶN khi ô lỗi; gia hạn từng bài), GiaoBaiTap (chiNgay, giờ máy, `hanNopISO` y cũ), ExamSetupScreen (giờ mở ca — LUỒNG THI: chỉ đổi ô nhập, `new Date(batDauLocal)`/kiểm giờ đã qua/ISO giữ nguyên, test khoá bằng nguồn), BangTinGiaoVien (chiNgay; nhập dở giữ ngày đang xem).
- [x] Mọi chỗ HIỆN hạn/giờ giao ở app giáo viên: "HH:mm · Thứ … dd/mm/yyyy" giờ VN (`gioDayDu`): PhanCong, `NhanHanBaiTap dayDu` (app HS/PH KHÔNG đổi), Bảng tin ("Hạn: 23:59 · Thứ Năm 24/09/2026" — thay `toLocaleString('vi-VN')` kiểu máy, lỗi thật trong ảnh thầy).
- [x] Bẫy đã bắt: tự nhảy ô kích blur TRƯỚC khi React vẽ lại ⇒ đọc state cũ làm "25" bị bù thành "02" — sửa bằng ref bản mới nhất (test gõ từng phím + đột biến).
- [x] 21 test + đột biến 9/9; tsc -b + build exit 0; m3-b/m3-c xanh; 8 test đỏ ở ExamSetup/mở ca giống HỆT HEAD (nền đỏ); ảnh `og-{giao,da-giao,mo-ca,bang-tin}-*.jpg` (12 tệp, 1440/390 sáng/tối, 0 lỗi tương phản/chạm/tràn). Test `btvn-giao-dien-han-nop` (không phải của em) sửa theo ô mới, giữ ý (hạn quá khứ ⇒ không gọi máy chủ).

## Hôm nay v2 — bước 1–4 (de5648e, eeff328, c5a621a, 5207efe; thầy lệnh 21/09, bản vẽ `docs/ban-ve-hom-nay-v2-2109/`)
- [x] B1–2 (de5648e): lưới + ô tra cứu lớn (`OTraCuu`) + ô VIỆC GẤP chưa nộp BTVN (`ViecGap`) + "Gửi cảnh báo" (lệnh GHI `/gv/canh-bao-nop-bai` chưa có ⇒ nói thật, KHÔNG giả "đã gửi"). Bỏ hẳn: Việc cần theo dõi, Bảng tin + ô ngày kiểu máy, Hoạt động dạy học, Bài đã giao.
- [x] Bộ não CHƯA có bản tin ⇒ thẻ TRUNG TÍNH (eeff328, Boss đã soát); đỏ chỉ khi >36 giờ không chạy hoặc lệnh lỗi thật.
- [x] B3 (c5a621a): `EmCanGiup` — lệnh CHỈ ĐỌC `/gv/can-giup`; từng dạng "sai x/y câu · bậc · đang giảm/đang lên/giữ nguyên 7 ngày" + trễ nhịp; lọc lớp; "Xem cả N em"; hành động [Đưa vào buổi chữa · Giao bài riêng]. Chưa có lệnh ⇒ lời thật + danh sách rút gọn cũ. Trả lời thiếu `ds` ⇒ báo không đọc được (KHÔNG nói "không ai cần giúp"). Cột A 1.62→1.25 theo bản vẽ.
- [x] B4 (5207efe): `VinhDanh` — lệnh `/gv/vinh-danh-ngay`; chỉ dựng bục có số; không số ⇒ thu gọn 1 dòng; "Chiếu vinh danh" = tấm phủ chữ to (chưa có màn chiếu vinh danh sẵn). Bục Tiến bộ thiếu số ⇒ null, không điền 0.
- [ ] B5 Toàn cảnh một em (kế). [ ] Xoá mã chết `BangTinGiaoVien.tsx` + `GiaoBaiTap.tsx` SAU B5 (Boss chốt).
- Kiểm: 12 + 7 test mới, đột biến 10/10 mỗi bước; Chromium 1440/390 sáng·tối 0 lỗi tương phản/chạm/tràn (`hn2-can-giup-*`, `hn2-vinh-danh-*`, `hn2-chieu-*`).

## BỎ PHIẾU ZALO (thầy lệnh 21/09 qua Boss; P26–P29 trong `docs/ra-soat-tu-ngu-2109-code2.md`)
- [x] GỠ hẳn khối "Gửi phụ huynh" ở hồ sơ em (tin chữ Zalo + ảnh phiếu + "Xem báo cáo" + "Thu hồi link") và nút "Nhắn phụ huynh" (hồ sơ em; ô Em cần thầy giúp ở Hôm nay — Boss chọn phương án A: kênh tới phụ huynh nay là "Gửi cảnh báo" + lời Bộ não A.I trong app phụ huynh; nhắn tay trong app để sau). Em trễ nhịp ở danh sách rút gọn nay có nút "Xem hồ sơ" (`hom-nay-api.hanhDongCua`).
- Tệp XOÁ: `src/components/PhieuZaloEm.tsx` (695 dòng) · `src/lib/phieu-zalo.ts` · `src/lib/anh-phieu.ts` (mồ côi) · tests: `phieu-zalo.test.ts`, `phieu-zalo-va-anh.test.tsx`, `zalo-khong-tin-o-diem-0809.test.ts`, `phieu-doc-o-sheet-khi-du-prop-0909.test.tsx`.
- Tệp SỬA: `HocSinhScreen.tsx` (import, state `hienZalo`, nút, khối) · `lib/phieu-hang-loat.ts` (bỏ ảnh phiếu → gói zip `goiPhieuCaZip`/`tenTepZipCa`, không màn nào gọi; giữ `chuyenDeTuChiTiet` + kiểu `ChuyenDeMatDiem`) · `ExamMonitorScreen.tsx` + `GiaoBaiTap.tsx` (chỉ 1 dòng import; LUỒNG THI không đổi chữ) · `hom-nay-api.ts`, `EmCanGiup.tsx`.
- Tệp MỚI: `src/lib/tin-nhan-thay.ts` = `soanTinRoiMan` (báo phụ huynh việc rời màn — gửi vào HỘP THƯ trong app, không phải Zalo) + `tinBaoBaiTap`, NGUYÊN VĂN; `tests/tin-nhan-thay.test.ts` (9 test chuyển nguyên) · `tests/chi-tiet-ca-cham-ten-em.test.tsx` (giữ 3 khoá không dính Zalo + 1 khoá mới: hồ sơ em KHÔNG còn PhieuZalo/hienZalo/Nhắn phụ huynh/Gửi phụ huynh).
- Test sửa CÓ CHỦ Ý: `phieu-link` (bỏ khối "tin nhắn Zalo mang link") · `de-rieng-nhan` · `ho-so-tu-mau-thuan-1009` (bỏ khối đọc mã PhieuZaloEm) · `rut-theo-dang-va-pham-vi-ca` (3b chỉ còn đường cả ca) · `link-phieu-ca` (chuyển khoá `loai` sang phieu-ca-ca) · `thieu-nut-copy-link` (bỏ phép 1–5, giữ 6–7) · `phieu-hang-loat` (bỏ "Tên tệp zip") · `khung-xem-phieu`, `m3-c6` (chú thích) · `doi-ten-hoc-sinh-2109`, `hai-nut-moi-em`, `hoc-sinh-m3-2109` (bỏ mock PhieuZaloEm + nút) · `hom-nay-2109`, `hom-nay-v2-em-can-giup-2109` (còn 2 nút).
- KHÔNG đụng: phiếu HTML app phụ huynh, `NutPhieuHtml`, link phiếu cả ca (`phieu-ca-ca`, `link-phieu-ca`), `KhungXemPhieu`, cầu nối Cowork, "vào thi từ Zalo" (chỉ là trình duyệt nhúng). Còn chữ "Zalo" ở lời hướng dẫn copy link (NutPhieuHtml, KhoiTroLy, tro-ly) — thuộc luồng LINK phiếu HTML, không phải phiếu Zalo; chờ Boss nếu muốn đổi chữ.
- Kiểm: `tsc -b` — 0 lỗi ngoài tệp DỞ của phiên khác (`nhiem-vu-adapter.ts`/`BangNhiemVu.tsx`, Code 2, chưa commit); 60 tệp test dính tới các tệp vừa đụng: 1024 xanh + 13 đỏ GIỐNG HỆT HEAD sạch (nền đỏ, đã diff tên); ảnh `zalo-go-hoc-sinh-hoso-{1440,390}-sang.jpg`, `hn2-can-giup-*` chụp lại.

## Hôm nay v2 — bước 5 Toàn cảnh một em (46a2b3b, 6f14512) · BẢN 1.2 phần Code 4 (5c6168e) · bản B khung cuộn + tự động nhắc (a758eef) — 21/09
- [x] B5 Toàn cảnh một em: màn `ToanCanhEmScreen` (id `toancanh`, thanh bên sáng "Hôm nay"), `lib/em-toan-canh.ts` (adapter `/gv/em-toan-canh`), `components/toan-canh/CacKhoi.tsx`. Đầu trang (tên · SBD · lớp · thần thú · chuỗi · hoạt động cuối · phụ huynh xem app · Giao bài riêng · Cho thi lại chỉ khi biết ca · Xem hồ sơ học sinh — KHÔNG "Nhắn phụ huynh", đã gỡ), dòng thời gian nhóm theo NGÀY VN + lọc loại (KHÔNG chip đếm số — máy chủ không trả; "Mở app" chỉ khi thấy thật) + "Xem thêm việc cũ hơn" (truoc/conNua), bản đồ dạng, nhịp học 30 ngày, thẻ phụ huynh. Mở từ: ô tra cứu · tên em ở Việc gấp / Em cần thầy giúp / Vinh danh / Bộ não. Chưa có lệnh ⇒ lời thật + vẫn hiện tên em từ danh sách lớp. 19 test + đột biến 14/16 (2 sót tương đương). Code 3 đồng ý 6 điểm dạng trường (mota/maCa/maBtvn, diemCaGanNhat{diem,tenCa,maCa,luc}…).
- [x] BẢN 1.2 (Xem trước): "bắt buộc N câu · thử sức thêm M câu (không bắt buộc)" mỗi em, cột đầu BẮT BUỘC khi máy chủ trả số, nhóm "Thử sức thêm" tách riêng SAU chặng cuối (nhãn "Câu cốt lõi · thử sức thêm (sai không sao)"), tab theo dõi "Bộ của em: bắt buộc … · chặng a/b". Tên trường theo TomTatBo Code 1; Code 3 (5b37676) xác nhận đúng `soBatBuoc/soThuSucThem` + `chiTiet.thuSucThem`. Chưa có trường ⇒ như cũ. Khoá chữ cũ sửa CÓ CHỦ Ý (nhãn loi_cao; chiTiet có `thuSucThem:[]`).
- [x] BẢN B: hai ô hàng A cao 560 (≥1100) + hàng B cùng cao; đầu dính + thân khung cuộn (`KhungCuon`: region có tên, focus, mép mờ); dòng em lưới 3 cột [40][1fr][148]; thẻ hẹp <420 (container query) nút xuống dưới; Việc gấp: bỏ nút đỏ lớn → "Tự động nhắc: ĐANG BẬT · lượt kế …" + đếm hôm nay + dải từng em + nút nhỏ "Nhắc ngay" (nút/lời chuyển "Gửi cảnh báo" → "Nhắc ngay"); Bộ não tiêu đề 1 dòng; Dạng cả lớp lưới 1fr/120/72. Adapter `/gv/chua-nop`: `tuDong{bat,lanKe}`, `homNay{soEm,soPhuHuynh}`, mỗi em `daNhac[{moc,luc,emDaXem,phDaXem|null}]` + `nhacKe` — Code 3 ĐỒNG Ý, chưa có mã ⇒ ô nói thật "Máy chủ chưa báo trạng thái tự động nhắc." Đột biến 16/16; bộ soi 0 lỗi (xoá CSS chết `.hn-tim`, G03); ảnh `hn2-khung-{1440,1100,390}-{sang,toi}.jpg` (tên 30 ký tự + dạng dài); tsc -b + build exit 0 ở worktree sạch.
- [ ] Còn: xoá mã chết `BangTinGiaoVien.tsx` + `GiaoBaiTap.tsx` (Boss: sau bước 5 — nay có thể); các cụm sửa chữ đã duyệt (hàng 2,3,5,6,7,9–15,18 của `docs/ra-soat-tu-ngu-2109-code4.md`); G01/G02/G04 theo cụm màn.

## Từ ngữ cụm 1–3 + xoá mã chết + TÊN LỚP (21/09; 3682fd3, 1a59705, 6d46e46, 0f9413a, 888f76e)
- [x] Xoá mã chết (3682fd3, Boss duyệt): `BangTinGiaoVien.tsx`, `GiaoBaiTap.tsx`, `game/than-thu-v2/Room.tsx` (không ai nhập; kiểm import + tsc -b sạch); bỏ khỏi danh sách quét m3, sinh lại `m3-tuong-thich.css`; test khoá nguồn sửa CÓ CHỦ Ý. Còn lại trong mã nhưng mồ côi: `lib/tin-nhan-thay.ts › tinBaoBaiTap` (chỉ GiaoBaiTap dùng) và bản `gon` của `KhoiBoNaoDemQua` (chỉ Bảng tin dùng) — để nguyên, chờ Boss quyết.
- [x] Từ ngữ cụm 1 (1a59705; hàng 2,3,5,6,7,11,12): lô→chặng; "Đợt bài đã giao"→"Bài tập về nhà đã giao"; lõi/riêng→"câu cốt lõi"/"câu dành riêng cho em" (cột "CÂU CỐT LÕI / DÀNH RIÊNG"; tên màn "Xem trước phân bổ" + "Cá nhân hoá" GIỮ); nhãn câu "Câu cốt lõi · Câu dành riêng cho em · Câu thử thách (sai không sao)"; KHÔNG hiện mã dạng/mã câu ("dạng chưa đặt tên", "chưa gắn dạng", "Câu không còn trong bài"); hộp xác nhận: nút theo việc ("Cho làm lại"/"Thu hồi bài") + "Giữ nguyên". Khoá mới `tests/tu-ngu-app-thay-2109.test.ts`.
- [x] Cụm 2 (6d46e46; hàng 8,10,15): nhật ký Bộ não "Khắc phục:"→"Ôn lại:" + tên dạng (`ten`/`tenDang` đã đề nghị Code 3; thiếu ⇒ "dạng chưa đặt tên (mã …)"); Ngân hàng đề "TN · Đ/S · TLN"→"Trắc nghiệm · Đúng–sai · Trả lời ngắn".
- [x] Cụm 3 (0f9413a; hàng 9,13,18): "bộ não"→"Bộ não A.I" ở mọi chữ hiện ra (35 chỗ); Gọi lên bảng dùng `gioDayDu` ("Hạn nộp 23:59 · Thứ … dd/mm/yyyy", đúng 2 dòng); bộ lọc Toàn cảnh "Gọi lên bảng".
- [ ] CÒN: hàng 14 (toLocale* rải ~40 chỗ → `gioDayDu`), hàng 1/4/16 KHÔNG đổi (Boss), lỗi bộ soi ở các tệp còn lại (G01/G02/G04).
- [x] TÊN LỚP (888f76e; thầy lệnh): "Chọn theo lớp" ở Giao bài tập về nhà (chip "12 - Tinh Hoa · 42 em" ⇒ chọn sẵn mọi em vào đường "Theo em" sẵn có, bỏ/tick từng em được, không đổi luật giao) + Học sinh: tên lớp thật ở danh sách/hồ sơ, chip lọc theo lớp, "Đổi lớp" một em. Lớp nối `lib/ten-lop-thay.ts`: `/gv/lop` (đọc) + `/gv/doi-lop-em {sbd, tenLop}` (ghi) — dạng ĐỀ NGHỊ đã nhắn Code 3 (hợp đồng docs/hop-dong-ten-lop-2109.md chưa thấy); 404/rỗng ⇒ mục ẩn, không lỗi đỏ. 16 test + đột biến 16/16; soi 0 lỗi (sửa G03/G04/G07/G08 cũ ở PhanCongScreen + HocSinhScreen); tsc -b + build exit 0 ở worktree sạch.

## Bộ soi + hàng 14 + mồ côi (21/09; 406fdc6, fa7cb4f, 6715cb4, 225d5a6, ae2a009, 3b44c24, 7640699)
- [x] Bộ soi làn thầy (Boss: "sửa dần G01–G04"): G04 `transition-all`→thuộc tính cần chuyển, G03 bỏ `outline: none` nội tuyến / thêm `focus-visible`, G01 `<div onClick>`→vai nút + Enter/Space, G02 `aria-label` bốn nút đóng ở ExamSetup. Tệp: DesignSystem, NhomCaThuGon, NutQuayLai, CauHoi, ClassList, LichSuCa, teacher-layout.css, KhoiMatKhauApp, KhoiMayChuMoi, KhoiRutDe, NutDongBoDanhSach, NutTaiDeCa, NutThemHocSinh, KhoaApp, NganHangDe, MessagesFab (406fdc6) · GoiLenBang (fa7cb4f — SỰ CỐ: cuốn nhầm ~126 dòng B6 của Code 1 vì họ Edit xen giữa; đã khép, luật thêm ở Nhật ký) · ExamSetup + ExamMonitor + exam-setup.css (6715cb4, chỉ className/aria-label/transition/outline, Boss soi từng dòng) · HoSoEmView, ModalXacNhanNop, HopChonDe, KhoiTroLy, TamTruotHoiBai, TheCau (3b44c24). Nay bộ soi LỖI = 0 ở mọi tệp làn thầy (toàn kho 76, còn lại của làn HS/PH/game).
- [x] Xoá mồ côi (225d5a6, Boss duyệt): `tinBaoBaiTap` + bản `gon` của KhoiBoNaoDemQua (props gon/soDongGon/onXemDayDu) — grep không còn nơi dùng.
- [x] Hàng 14 (ae2a009): `ngayDayDu` mới + `gioDayDu`/`gioPhutVN` cho Ngân hàng đề, Trợ lý phụ huynh, MessagesFab, Lịch sử ca, Câu hỏi, HoSoEmView, KhoiThoiGianCa; danh sách gọn giữ "dd/mm HH:mm"; luồng thi/Gọi lên bảng/HS-PH/game không đụng.
...
- [x] 21/09 Boss soát 757a9da+b4fbd6d ĐẠT; 3 quyết định: (1) 8c4ab80 "ca thi"→"ca kiểm tra" ở chữ hiển thị/aria Mở ca + Theo dõi ca (+KhoiThoiGianCa aria); test khoá chữ cũ sửa có chủ ý: phong-cho.test.ts ("Huỷ ca thi"→"Huỷ ca kiểm tra"); nhóm 51 tệp: 8 đỏ = nền. Chuẩn từ ngữ dòng 21 (giữ "Ca thi" ở app thầy) nay LỖI THỜI cho hai màn này — Boss cập nhật. (2) 430951c chip Mở ca bị cắt "…" ⇒ xuống 2 dòng, cỡ chữ giữ 13 px, ảnh trước–sau 360/390 ở docs/anh-app-giao-vien-2109/mo-ca-chip-*.jpg. (3) Kiểm sống /gv/lop: KHÔNG dùng phiên thầy; mô phỏng đủ, Boss nhờ thầy nhìn.
