...
Thú của em nằm ở `thanThu` của thẻ: `ten` (được nhắc) và `chuoiNgay`. TẠM CẤM nêu số EXP còn thiếu và số cấp (máy ẩn `cap`, `expConThieu`, `exp` khỏi thẻ bạn đọc, và loại mọi lời có "40 EXP", "cấp 6"…; thầy đang tính lại đường cấp); khiên / mảnh khiên vẫn TẠM KHÔNG dùng. Khoá vắng = không có số thật ấy; vắng cả khối = em chưa chọn thú ⇒ KHÔNG nhắc thú, KHÔNG nêu tên riêng nào; NÊN đặt ở CÂU CUỐI một câu mời chọn thú ("chọn một thần thú" / "chọn thần thú" / "chọn một bạn đồng hành"), không tên thú, không hứa EXP cụ thể (tối 21/09: 35/60 em chưa chọn thú). Tên thú chỉ lấy đúng từ `thanThu.ten`.
**Khung lời mời — BA Ý BẮT BUỘC, đủ cả ba** (lượt thử 12:31 tối 21/09: 52/52 lời đúng số nhưng cụt ý giữa, 47/52 mở bằng "Hôm qua"): (1) điều em đã làm bằng số thật (số "đúng" = số đã gặp − số sai của dạng) → (2) Ý NGHĨA với chính em: MỘT mệnh đề ngắn nói điều đó CHO THẤY GÌ ở em, dựa dữ kiện, CẤM khen chung chung → (3) lời mời có lực ("Hôm nay em hãy…", "Thử ngay mấy câu…", "Cùng <tên thú> luyện…") gắn với thú bằng TÊN thú (không nêu số EXP / cấp). Không hứa điều không chắc: EXP chỉ có khi trả lời đúng, máy chủ có thể chọn được ít câu hơn. Mỗi em một lời KHÁC nhau: ≥ 4 kiểu mở đầu, không mở bằng "Hôm qua" quá 1/3 số lời, mỗi lời một đuôi khác nhau (máy cảnh báo cả lô khi quá nửa "Hôm qua", < 4 kiểu mở đầu hoặc đuôi lặp).
...
1. Sai rồi sửa lại được 4 câu, đó là cách nhớ lâu nhất. Hôm nay em hãy cùng Rồng Lửa thử mấy câu Thuỷ phân ester nhé.
2. Chuỗi 4 ngày liền, hôm nay đến lúc nâng tầm: hãy thử câu khó hơn một bậc ở dạng Thuỷ phân ester, vì em đã đúng 8 trong 9 câu dạng này. Rồng Lửa đang chờ em cùng thử.
3. Hôm qua em làm 8 câu, đúng 7 câu, nhịp đều như vậy là nền vững cho bài dài. Thử ngay vài câu Lipid béo cùng Rồng Lửa nhé.
4. Rồng Lửa đang chờ em cùng luyện tiếp. Em vừa đúng lại 4 câu từng sai, nên hôm nay hãy thử mấy câu Thuỷ phân ester để chắc thêm.
...
**8 lời mẫu CẤM** (máy sẽ loại cả thử thách):
...
2b. Rồng Lửa còn thiếu 40 EXP để lên cấp 6, hôm nay thử mấy câu nhé. — nêu số EXP hoặc số cấp (tạm cấm).
