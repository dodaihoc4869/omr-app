...
it('chờ bấm Lên bảng, sang trang rồi quay lại phải bấm mới',()=>{
 ...
 expect(heads()).toEqual([true,true]);now=61000;tick();expect(heads()).toEqual([true,true]);doc.getElementById('mc-len-bang')!.click();expect(heads()).toEqual([false,false])
 doc.getElementById('mc-sau')!.click();expect(doc.getElementById('mc-clock')).toBeNull()
 doc.getElementById('mc-truoc')!.click();expect(heads()).toEqual([true,true]);expect(doc.getElementById('mc-clock')).toBeNull();dom.window.close()
})
it('hiện thần thú toàn màn hình rồi dọn hiệu ứng khi chuyển lượt',()=>{
 ...
 const doc=dom.window.document;now=61000;tick();expect(doc.querySelectorAll('.mc-intro-card')).toHaveLength(0);doc.getElementById('mc-len-bang')!.click()
