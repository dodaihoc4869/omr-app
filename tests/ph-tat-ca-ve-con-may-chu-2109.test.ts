...
describe('ca CHƯA công bố: chỉ congBo + soEmDaNop/soEmDaVao — không điểm, không số câu, không đáp án', () => {
  ...
  async function dungCa(congBo: string | null, trangThaiCa: string, s2Nop: boolean) {
    ...
    await dungLaiHoSo(d.env, ['S1'], new Date(NOW).toISOString())
...
describe('lichOn và phuHuynhLamGi — số thật từ hồ sơ, xưng "con"', () => {
  ...
  it('em chỉ có câu sai của ca CHƯA công bố ⇒ lịch ôn không đếm chúng (không lộ số câu sai qua "Nhắc con làm N câu")', async () => {
    ...
    await dungLaiHoSo(d.env, ['S1'], new Date(NOW).toISOString()) // hồ sơ dựng từ sổ ĐẦY ĐỦ: nam_kt_cau có 6 câu sai tới hạn hôm nay
...
describe('bảng kiểu Apple — trường mới của lệnh /ph/tat-ca-ve-con', () => {
  ...
  it('lichOn.tongTungSai = đã khắc phục + còn sai; bayNgayToi = 7 ngày (mai … +7) kẹp bằng trần ôn; phutNgayMai chỉ khi ngày mai có câu', async () => {
    ...
    await dungLaiHoSo(d2.env, ['S1'], new Date(NOW).toISOString())
