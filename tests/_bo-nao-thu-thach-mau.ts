 ...
 * Tám mẫu, tám kiểu MỞ ĐẦU khác nhau; hai mẫu đẩy bậc (`cao_hon_mot_bac`, `thap_hon_mot_bac`) nói ĐÚNG bậc; thẻ có `thanThu` chỉ nhắc TÊN thú (TẠM KHÔNG nêu số EXP / cấp; KHÔNG khiên); em chưa có thú thì mời chọn thú ở CÂU CUỐI (ba cách nói). */
export const LOI_TOT: { chu: string; the: TheDeKiem; dang: string[]; bac: 'dung_bac' | 'thap_hon_mot_bac' | 'cao_hon_mot_bac' }[] = [
  ...
  { chu: 'Chuỗi 4 ngày liền, hôm nay đến lúc nâng tầm: hãy thử câu khó hơn một bậc ở dạng Thuỷ phân ester, vì em đã đúng 8 trong 9 câu dạng này. Rồng Lửa đang chờ em cùng thử.', the: THE, dang: ['ESTE.THUY_PHAN'], bac: 'cao_hon_mot_bac' },
  { chu: 'Hôm qua em làm 8 câu, đúng 7 câu, nhịp đều như vậy là nền vững cho bài dài. Thử ngay vài câu Lipid béo cùng Rồng Lửa nhé.', the: THE, dang: ['LIPID.BEO'], bac: 'dung_bac' },
  { chu: 'Rồng Lửa đang chờ em cùng luyện tiếp. Em vừa đúng lại 4 câu từng sai, nên hôm nay hãy thử mấy câu Thuỷ phân ester để chắc thêm.', the: THE, dang: ['ESTE.THUY_PHAN'], bac: 'dung_bac' },
...
export const LOI_CAM: { chu: string; mau: string; bac?: 'dung_bac' | 'thap_hon_mot_bac' | 'cao_hon_mot_bac' }[] = [
  ...
  { chu: 'Rồng Lửa còn thiếu 40 EXP để lên cấp 6, hôm nay thử mấy câu nhé.', mau: 'nêu số EXP hoặc số cấp' },
