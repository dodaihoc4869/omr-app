...
describe('Đảo thần thú · lý do thưởng (chữ sẵn in, khớp mốc 20/40/40 của advance)', () => {
  it('bảng', () => {
    ...
    expect(lyDoThuong({ correct: true, assisted: false, reward: 40, milestone: 2, stage: 2 })).toMatchObject({ moc: 2, exp: 40 }); expect(lyDoThuong({ correct: true, assisted: false, reward: 40, milestone: 3, stage: 3 }).chu).toContain('sao thứ 3')
