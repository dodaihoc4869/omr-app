...
import {allowed,advance,chooseSession,grade,newArena,arenaAction,PETS,DAY,targetLevel,publicQuestion} from '../src/game/than-thu-v2/core'
...
describe('Phạm vi và tiến bộ thật',()=>{
 ...
 it('20/40/40 chỉ qua câu khác nhau và thời gian, không farm',()=>{const first=advance(undefined,attempt());expect(first.reward).toBe(20);expect(advance(first.mastery,attempt(DAY+1)).reward).toBe(0);expect(advance(first.mastery,attempt(2,'g1')).reward).toBe(0);const second=advance(first.mastery,attempt(DAY+1,'g1'));expect(second.reward).toBe(40);expect(advance(second.mastery,attempt(6*DAY,'g2')).reward).toBe(0);const third=advance(second.mastery,attempt(7*DAY+1,'g2'));expect(third.reward).toBe(40);expect(third.mastery.repaired).toBe(true);expect(advance(third.mastery,attempt(14*DAY,'g3')).reward).toBe(0)})
...
describe('D1 thực: tách dữ liệu, phân quyền, giao dịch',()=>{
 ...
 it('ống đầy và cấp 120 giữ phần EXP còn dư',async()=>{const f=fixture();f.sql.prepare('INSERT INTO than_thu VALUES(?,?)').run('1',JSON.stringify({idThanhThuChon:'dat_quy',capDo:119,exp:thanhExp(119)-5,khoExp:50000}));const token=await gameToken(f.env,'1');await gameV2(f.env,'invest',{token});const p=(await loadProfile(f.env,'1')).profile;expect(p.cap).toBe(120);expect(p.wallet).toBe(49995)})
...
describe('EXP học tập đồng bộ',()=>{
 it('chặn cộng trùng, trần ngày VN, không kéo điểm trước reset, tự nạp cấp',async()=>{
  ...
  expect(creditAcademic(p,[{key:'tomorrow',at:'2026-09-16T18:00:00Z',amount:30}])).toBe(30);expect(p.cap).toBe(2)
 })
