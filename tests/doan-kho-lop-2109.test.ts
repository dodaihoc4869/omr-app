...
describe('kho LỚP cho câu cá nhân của Đoàn', () => {
  it('em MỚI (chưa bằng chứng, chưa bài) vào Đoàn được: 6 câu cá nhân KHÁC nhau, Phần I/III, đều thuộc dạng lớp đã học (A.1), không câu dạng chưa ai học, không Phần II', async () => {
    const d = dung()
  ...
  it('kho lớp ÍT câu Phần I (4 câu A.1 + 3 câu Phần II cùng dạng, dạng B.2 chưa ai học có 10 câu): chỉ nhận 4 câu Phần I của dạng lớp — không bù bằng Phần II, không bù bằng dạng chưa ai học', async () => {
  ...
  it('không dạng lớp nào và em không bằng chứng ⇒ báo "Chưa có câu vừa sức" (không rút bừa từ dạng chưa ai học)', async () => {
    ...
    await expect(goi(d, 'S1', 'mo')).rejects.toThrow(/Chưa có câu vừa sức/)
  })
})
