...
describe('MỐC: biên đúng ở mọi nguồn (04:59:59Z không đếm, 05:00:00Z đếm)', () => {
  ...
  it('bài tập: giao 04:59:59Z không hiện, giao 05:00:00Z hiện; nhắc tự động gui_luc 04:59:59Z không đếm', async () => {
    const d = truong()
    themBai(d, 'B-TRUOC', '2026-09-21T04:59:59.000Z', T('2026-09-24T12:00:00')); themBaiEm(d, 'B-TRUOC', EM[0]!)
    themBai(d, 'B-DUNG', '2026-09-21T05:00:00.000Z', T('2026-09-24T12:00:00')); themBaiEm(d, 'B-DUNG', EM[0]!); themBaiEm(d, 'B-DUNG', EM[1]!)
    ...
    const r = await goi(d)
    expect((r.baiTap as { maBtvn: string }[]).map((b) => b.maBtvn)).toEqual(['B-DUNG'])
    expect(r.baiTap[0].nhac).toMatchObject({ soEm: 1, soPhuHuynh: 1 })
