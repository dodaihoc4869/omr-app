...
describe('T33 — bốn adapter chấm đi CÙNG một policy, không lệch nhau', () => {
  ...
  it('khopPhanIII (ca thi + mọi màn), isAnswerCorrect (BTVN/ôn lại), chamMotCau, gradeMom cùng kết quả', () => {
    for (const [em, dapAn] of CAP) {
      ...
      expect(chamMotCau({ qid: 'SYN-CNH-III-1', dapAn: em }, { correct: dapAn, phan: 'III' }), `chamMotCau ${JSON.stringify(em)}`).toBe(mongDoi)
  ...
  it('numeric server keys outside grammar throw material errors; boolean legacy adapters fail closed', () => {
    ...
    for (const em of ['SO3', 'SO2']) {
      ...
      expect(chamMotCau({ qid: 'SYN-CNH-III-1', dapAn: em }, { correct: 'SO3', phan: 'III' })).toBe(false)
    }
