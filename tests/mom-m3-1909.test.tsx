...
describe('C4 · ĐANG LÀM BÀI ở cổng học sinh (/hs)', () => {
  ...
  it('"Tạm dừng": lưu nháp kèm số giây còn lại, báo, về danh sách; "Danh sách bài": hỏi xác nhận rồi về danh sách (huỷ thì ở lại)', async () => {
    ...
    // Hộp riêng thay confirm/alert của trình duyệt: tiêu đề nói việc, nút mang động từ.
    fireEvent.click(screen.getByRole('button', { name: 'Danh sách bài' }))
    const hoi = screen.getByRole('alertdialog', { name: 'Rời bài đang làm?' })
    expect(hoi.textContent).toContain('Đồng hồ 2 tiếng vẫn tiếp tục đếm ngược.')
    fireEvent.click(within(hoi).getByRole('button', { name: 'Làm tiếp' }))
    expect(screen.queryByRole('alertdialog')).toBeNull()
    expect(document.querySelector('.mom-dong-ho')).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: 'Tạm dừng' }))
    const bao = screen.getByRole('alertdialog', { name: 'Đã lưu nháp' })
    expect(bao.textContent).toContain('Em vào làm tiếp bất cứ lúc nào trước Hạn nộp.')
    fireEvent.click(within(bao).getByRole('button', { name: 'Đã hiểu' }))
