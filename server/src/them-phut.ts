...
//   · mọi lượt ĐANG LÀM của ca có `het_gio_luc` thì `het_gio_luc += phut`,
...
//
// MỘT batch, hai câu, cùng điều kiện tiên quyết "`them_phut_tong` vẫn bằng giá trị vừa đọc" (khoá lạc quan): hai lệnh chạy chồng nhau thì chỉ một lệnh có tác dụng, lệnh kia
// không đổi gì và báo thử lại — không bao giờ lệch giữa giờ của ca và giờ của lượt. Câu lượt đi TRƯỚC câu ca (cả hai đọc cùng điều kiện lúc bắt đầu batch).
...
export async function themPhutCa(env: Env, b: Record<string, unknown>, nowMs: number = Date.now()): Promise<Record<string, unknown>> {
  ...
  const cau: D1PreparedStatement[] = [
    ...
    // 2. Ca: cộng phút và tổng đã thêm (cùng điều kiện tiên quyết).
    env.DB.prepare(
  ...
  return { ok: true, phut, soLuotCong, thoiGianPhut: Number(sau?.thoi_gian_phut) || 0, themPhutTong: Number(sau?.them_phut_tong) || 0 }
}
