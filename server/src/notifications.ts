...
export async function deliverNotices(env:Env){
 ...
 const rows=(await env.DB.prepare(`SELECT n.id,p.endpoint,p.subscription FROM student_notice n JOIN student_push p ON p.sbd=n.sbd LEFT JOIN student_push_delivery d ON d.notice_id=n.id AND d.endpoint=p.endpoint WHERE n.created_at>=p.created_at AND n.created_at>strftime('%Y-%m-%dT%H:%M:%fZ','now','-1 day') AND d.sent_at IS NULL AND COALESCE(d.attempts,0)<3 AND COALESCE(d.lease_until,0)<? AND (
 ...
 OR EXISTS(SELECT 1 FROM mom_bai m WHERE m.sbd=n.sbd AND m.submitted_at IS NULL AND n.id='mom:'||m.sbd||':'||m.id)) LIMIT 40`).bind(Date.now()).all<{id:string;endpoint:string;subscription:string}>()).results
