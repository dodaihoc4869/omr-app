...
function Thanh({ dung, tong, nhan, xanh = true }: { dung: number; tong: number; nhan: string; xanh?: boolean }) {
  ...
  return (
    <span className={`mt-thanh${xanh ? ' mt-thanh--xanh' : ''}`} role="img" aria-label={nhan}>
...
function KhoiTongQuan({ pm }: { pm: PhMoi }) {
  ...
  return (
    <section className="mt-muc" id="tong-quan" aria-labelledby="h-1">
      <h2 id="h-1" className="mt-sr">
        Tổng quan hôm nay
      </h2>
      <div className="mt-hero">
        <p className="mt-hero__tren mt-so">
          {thuNgayVn(pm.serverNow ?? Date.now())}
          {pm.serverNow !== null ? ` · cập nhật lúc ${gioVn(pm.serverNow)}` : ''}
        </p>
        {dat !== null && (
          <div className="mt-hero__dat">
            <div className="mt-vong" role="img" aria-label={dat ? 'Nhiệm vụ hôm nay: đã đạt' : 'Nhiệm vụ hôm nay: chưa đạt'}>
              ...
              <div className="mt-vong__giua">{dat ? <Check className="mt-i mt-i--l" aria-hidden="true" /> : <Clock className="mt-i mt-i--l" aria-hidden="true" />}</div>
            </div>
            <div className="mt-hero__loi">
        ...
        {o.length > 0 && (
          <div className="mt-so-luoi">
            {o.map((x) => (
              <div className="mt-so-o" key={x.nhan}>
                <span className="mt-so-o__nhan">{x.nhan}</span>
                <b>
                  {x.b}
                  <small>{x.don}</small>
                </b>
                {x.phu && <span className="mt-so-o__phu">{x.phu}</span>}
              </div>
            ))}
          </div>
        )}
        {dong.length > 0 && (
          <div className="mt-hom-qua">
        ...
        {pm.doCham && (
          <p className="mt-hero__tren mt-so" data-vung="do-cham">
...
function KhoiDongThoiGian({ pm, nhom, onChonMoc }: { pm: PhMoi; nhom: NhomCau[]; onChonMoc: (id: string) => void }) {
  ...
  return (
    <section className="mt-muc mt-muc--7" id="dong-thoi-gian" aria-labelledby="h-2">
      <div className="mt-muc__dau">
        <h2 id="h-2">Dòng thời gian trong ngày</h2>
        <p>chạm một mốc để xem từng câu</p>
      </div>
      <div className="mt-the">
        <div className="mt-ngay" role="img" aria-label={`Con học ${moc.length} lần trong ngày: ${moc.map((m) => `${gioVn(m.batDau)} đến ${gioVn(Date.parse(m.batDau) + m.phut * 60_000)}`).join(', ')}`}>
          <div className="mt-ngay__thanh">
            {moc.map((m, i) => (
              <i key={i} style={pos(m)} />
            ))}
          </div>
          <div className="mt-ngay__nhan" aria-hidden="true">
        ...
        <ol className="mt-tl">
          {moc.map((m, i) => {
            ...
            return (
              <li key={i}>
                <button type="button" className="mt-tl__moc" onClick={() => n && onChonMoc(n.id)} aria-label={`${gioVn(m.batDau)}, ${ten}: ${chu}. Xem từng câu của mốc này`}>
                  <span className="mt-tl__gio">{gioVn(m.batDau)}</span>
                  <span className="mt-tl__nut">{m.che ? <Lock className="mt-i mt-i--s" aria-hidden="true" /> : <Check className="mt-i mt-i--s" aria-hidden="true" />}</span>
                  <div className="mt-tl__chu">
                    <h3>{ten}</h3>
                    <p>{chu}</p>
                    {!m.che && m.soCau !== null && m.soDung !== null && m.soCau > 0 && <Thanh dung={m.soDung} tong={m.soCau} nhan={`đúng ${m.soDung} trong ${m.soCau} câu`} />}
                  </div>
                  <ChevronRight className="mt-i mt-i--s mt-tl__mui" aria-hidden="true" />
                </button>
              </li>
            )
          })}
...
function KhoiCa({ pm }: { pm: PhMoi }) {
  ...
  return (
    <section className="mt-muc mt-muc--5" id="ca-kiem-tra" aria-labelledby="h-3">
      <div className="mt-muc__dau">
        <h2 id="h-3">Ca kiểm tra gần nhất</h2>
      </div>
      {kq && kq.tong !== null ? (
        <div className="mt-the mt-ca">
          <div className="mt-ca__chu">
            <p className="mt-ca__diem" role="img" aria-label={`Điểm ${soVn(kq.tong)} trên 10`}>
              <b>{soVn(kq.tong)}</b>
              <span>/10 điểm</span>
            </p>
            ...
            {ca.truoc && ca.truoc.tong !== null && (
              <span className="mt-ss">
                <TrendingUp className="mt-i mt-i--s" aria-hidden="true" />
                <span className="mt-so">
          ...
          {ca.phan.length > 0 && (
            <div className="mt-phan-ds">
              {ca.phan.map((p) => (
                <div className="mt-phan" key={p.ma}>
                  <span className="mt-phan__ten">{p.ma === 'I' ? 'Phần I · Trắc nghiệm' : p.ma === 'II' ? 'Phần II · Đúng–sai' : 'Phần III · Trả lời ngắn'}</span>
                  {p.diem !== null && <span className="mt-phan__diem">{soVn(p.diem)}</span>}
                  <Thanh dung={p.dung} tong={p.tong} nhan={`${p.ma === 'I' ? 'Phần I' : p.ma === 'II' ? 'Phần II' : 'Phần III'}: đúng ${p.dung} trên ${p.tong} câu`} xanh={false} />
                  <span className="mt-phan__dem">
        ...
        <div className="mt-the mt-ca">
          <div className="mt-chua-cb" data-vung="chua-cong-bo">
            <span className="mt-chua-cb__bt">
              <Lock className="mt-i" aria-hidden="true" />
            </span>
            <div>
...
function ChipBac({ bac, moi }: { bac: 'biet' | 'hieu' | 'van_dung' | null; moi?: boolean }) {
  ...
  return (
    <span className="mt-bac" role="img" aria-label={`Bậc hiện tại của con: ${NHAN_BAC[bac]}${moi ? ' (mới lên hôm nay)' : ''}. Ba bậc: Biết, Hiểu, Vận dụng`}>
      {TEN_BAC_SO.map((t, i) => (
...
function KhoiManhYeu({ pm }: { pm: PhMoi }) {
  ...
  const the = (tieuDe: string, phu: string, ds: typeof m.lamTot, vap: boolean) =>
    ds.length > 0 ? (
      <div className={`mt-the mt-dang-the${vap ? ' mt-dang-the--vap' : ''}`} data-vung={vap ? 'dang-vap' : 'dang-tot'}>
        <div className="mt-dang-the__dau">
          <span className="mt-dang-the__bt">{vap ? <Flame className="mt-i" aria-hidden="true" /> : <Check className="mt-i" aria-hidden="true" />}</span>
          <div>
            <h3>{tieuDe}</h3>
            <p>{phu}</p>
          </div>
        ...
        <ul>
          {ds.map((d) => (
            <li className="mt-dang" key={d.tenDang}>
              <p className="mt-dang__ten">{d.tenDang}</p>
              <div className="mt-dang__hang">
                <span className="mt-dang__dem">
                  đúng{' '}
                  <b>
                    {d.dung}/{d.tong}
                  </b>{' '}
  ...
  return (
    <section className="mt-muc" id="manh-yeu" aria-labelledby="h-4">
      <div className="mt-muc__dau">
        <h2 id="h-4">Điểm mạnh · Điểm cần luyện</h2>
        <p>tính trên 14 ngày gần đây</p>
      </div>
      {len.length > 0 && (
        <div className="mt-len-bac" data-vung="len-bac">
          <span className="mt-len-bac__bt">
            <TrendingUp className="mt-i" aria-hidden="true" />
          </span>
          <div>
...
function ChiTiet({ ct }: { ct: ChiTietCau }) {
  if (ct.kieu === 'tu_choi')
    return (
      <p className="mt-ghi-chu-ve" role="alert" data-vung="loi-giai">
        {ct.chu}
      </p>
    )
  return (
    <>
      <p className="mt-cau__cau">
        <ChemText text={ct.de} />
      </p>
          ...
          {ct.phuongAn.map((p, i) => {
            ...
            return (
              <li key={i} data-la={dung ? 'dung' : undefined}>
                ...
                {dung && (
                  <small>
                    <Check className="mt-i mt-i--s" aria-hidden="true" />
                    Đáp án
                  </small>
                )}
              </li>
...
function DongCau({ c, sbd, tenNguon, mo }: { c: CauChe | CauThuong; sbd: string; tenNguon: string; mo: boolean }) {
  ...
  if (laChe(c)) {
    return (
      <div className="mt-cau" data-vung="cau-che">
        <div className="mt-cau__hang">
          <span className="mt-cau__gio">
            <b>{gioVn(c.luc)}</b>
            <span>{tenNguon}</span>
          </span>
          <span className="mt-cau__kq">
            <span className="mt-chip mt-chip--khoa">
              <Lock className="mt-i mt-i--s" aria-hidden="true" />
              Đã làm
            </span>
          </span>
          <span className="mt-cau__de">
            <span className="mt-cau__cau">{CHU_CHE[c.che]}</span>
          </span>
          {c.giay !== null && (
            <span className="mt-cau__chan">
              <span className="mt-cau__tg-o">
                <span className="mt-cau__tg">
                  <Hourglass className="mt-i mt-i--s" aria-hidden="true" />
                  {chuThoiGian(c.giay)}
                </span>
              </span>
            </span>
  ...
  const kq = c.dung === true ? { l: 'ok', t: 'Đúng', i: <Check className="mt-i mt-i--s" aria-hidden="true" /> } : c.dung === false ? { l: 'sai', t: 'Sai', i: <X className="mt-i mt-i--s" aria-hidden="true" /> } : { l: 'cho', t: 'Đã làm', i: null }
  return (
    <details className="mt-cau" data-vung="cau" data-kq={kq.l} open={mo || undefined}>
      <summary>
        <span className="mt-cau__gio">
          <b>{gioVn(c.luc)}</b>
          <span>{tenNguon}</span>
        ...
        <span className="mt-cau__kq">
          <span className={`mt-chip mt-chip--${kq.l}`}>
            {kq.i}
            {kq.t}
          </span>
          <ChevronDown className="mt-i mt-i--s mt-mui" aria-hidden="true" />
        </span>
        <span className="mt-cau__de">
          {c.tenDang && <span className="mt-cau__dang">{c.tenDang}</span>}
          <span className="mt-cau__cau">
            <ChemText text={c.de} />
          </span>
        </span>
        <span className="mt-cau__chan">
          {(c.conChon || c.dapAn) && (
            <span className="mt-cau__chon">
              {c.conChon ? (
                <>
      ...
      <div className="mt-cau__mo">
        {ct && <ChiTiet ct={ct} />}
        {/* Nút chỉ có khi máy chủ đã gửi mã câu VÀ báo có lời giải; mã câu KHÔNG bao giờ hiện ra màn. */}
        {!ct && c.coLoiGiai && c.qid && (
          <button type="button" className="mt-nut mt-nut--chu" data-vung="xem-loi-giai" disabled={dang} onClick={() => void lay(c.qid)}>
            {dang ? 'Đang lấy lời giải…' : 'Xem lời giải'}
          </button>
        )}
        {loi && (
          <p className="mt-ghi-chu-ve" role="alert">
            {loi}
          </p>
        )}
        {!ct && !(c.coLoiGiai && c.qid) && <p className="mt-mo-ta">Câu này chưa có lời giải để xem.</p>}
      </div>
    </details>
  )
}
...
function KhoiTungCau({ pm, nhom, sbd, mo, dongMo, batMo }: { pm: PhMoi; nhom: NhomCau[]; sbd: string; mo: Set<string>; dongMo: (id: string) => void; batMo: (id: string) => void }) {
  ...
  return (
    <section className="mt-muc" id="tung-cau" aria-labelledby="h-5">
      <div className="mt-muc__dau">
        <h2 id="h-5">Từng câu con đã làm hôm nay</h2>
        <p>{tatCa.length} câu · xếp theo giờ làm</p>
      </div>
      <div className="mt-loc" role="group" aria-label="Lọc danh sách câu">
        {loc3.map((f) => (
      ...
      <p className="mt-chu-giai">
        <span>
          <i className="mt-cham" />
          ô đặc: đúng
        </span>
        <span>
          <i className="mt-cham mt-cham--sai" />
          ô gạch chéo: sai
        </span>
      ...
      {hienNhom.map((n) => {
        ...
        return (
          <section className="mt-moc" id={n.id} key={n.id} aria-label={`${n.gio} · ${n.ten}`} data-vung="nhom-cau">
            <h3 className="mt-moc__h">
              <button type="button" className="mt-moc__dau" aria-expanded={dangMo} aria-controls={`${n.id}-ds`} onClick={() => (dangMo ? dongMo(n.id) : batMo(n.id))}>
                <span className="mt-moc__bt">{n.cau.every(laChe) ? <Lock className="mt-i mt-i--s" aria-hidden="true" /> : <Check className="mt-i mt-i--s" aria-hidden="true" />}</span>
                <span className="mt-moc__ten">
                  {n.gio} · {n.ten}
                </span>
                <span className="mt-moc__so">{[`${n.cau.length} câu`, dung + sai > 0 ? `đúng ${dung}` : '', sai > 0 ? `sai ${sai}` : '', n.phut ? `${n.phut} phút` : ''].filter(Boolean).join(' · ')}</span>
                <ChevronDown className="mt-i mt-i--s mt-mui" aria-hidden="true" />
                <span className="mt-moc__cham" role="img" aria-label={`Lần lượt ${n.cau.length} câu: ${n.cau.map((c, i) => `câu ${i + 1} ${c.kieu === 'che' ? 'chưa công bố' : c.dung === true ? 'đúng' : c.dung === false ? 'sai' : 'đã làm'}`).join(', ')}`}>
                  {n.cau.map((c, i) => (
                    <i key={i} className={`mt-cham${c.kieu === 'che' ? ' mt-cham--cho' : c.dung === false ? ' mt-cham--sai' : ''}`} />
                  ))}
                </span>
              </button>
            </h3>
...
function KhoiBtvn({ pm }: { pm: PhMoi }) {
  ...
  return (
    <section className="mt-muc mt-muc--6" id="btvn" aria-labelledby="h-6">
      <div className="mt-muc__dau">
      ...
      <div className="mt-the mt-btvn">
        {b.dangChay.map((d) => (
          <div key={d.maBtvn} data-vung="btvn-dang-chay">
            <p className="mt-nhom-ten">Đang chạy</p>
            <div className="mt-btvn__dau">
              <h3>{d.ten}</h3>
              {d.changTong !== null && d.changXong !== null && d.changTong > 0 && (
                <span className="mt-chip mt-chip--cho mt-so">
                  Xong {d.changXong} trong {d.changTong} chặng
                </span>
              )}
            </div>
            {d.changTong !== null && d.changXong !== null && d.changTong > 0 && d.changTong <= 12 && (
              <ol className="mt-chang" aria-label={`Con đã xong ${d.changXong} trong ${d.changTong} chặng`}>
            ...
            <p className="mt-dong-bt">
              <Clock className="mt-i" aria-hidden="true" />
              <span>
        ...
        {b.gan.length > 0 && (
          <>
            <p className="mt-nhom-ten mt-btvn__ngan">Gần đây · mới nhất trước</p>
            <ul className="mt-btvn-ds">
              {b.gan.map((g) => (
                <li className="mt-btvn-dong" key={g.maBtvn} data-vung="btvn-gan">
                  <div>
                    <h3>{g.ten}</h3>
                    <p>
                    ...
                    {g.dungHan !== null && (
                      <span className={`mt-chip ${g.dungHan ? 'mt-chip--ok' : 'mt-chip--luu-y'}`}>
                        {g.dungHan ? <Check className="mt-i mt-i--s" aria-hidden="true" /> : <Clock className="mt-i mt-i--s" aria-hidden="true" />}
                        {g.dungHan ? 'Nộp đúng hạn' : 'Nộp sau hạn'}
                      </span>
                    )}
                  </div>
                  {g.diem !== null && (
                    <div className="mt-btvn-dong__diem">
                      <b>{soVn(g.diem)}</b>
                      <small>/10 điểm</small>
                    </div>
                  )}
...
function KhoiLichOn({ pm }: { pm: PhMoi }) {
  ...
  return (
    <section className="mt-muc mt-muc--6" id="lich-on" aria-labelledby="h-7">
      <div className="mt-muc__dau">
        <h2 id="h-7">Lịch ôn lại</h2>
        <p>các câu con từng làm sai</p>
      </div>
      <div className="mt-the mt-on">
        <p className="mt-on__mai">
          <b>{l.ngayMai}</b>
          <span>
...
function KhoiNhip({ pm }: { pm: PhMoi }) {
  ...
  return (
    <section className="mt-muc" id="nhip-14-ngay" aria-labelledby="h-8">
      <div className="mt-muc__dau">
      ...
      <div className="mt-the">
        <ol className="mt-14" aria-label={`Số câu con làm mỗi ngày, từ ${ngayChuoiNgan(ds[0]!.ngay)} đến ${ngayChuoiNgan(ds[13]!.ngay)}`}>
          {ds.map((d, i) => {
        ...
        <p className="mt-thang-mau" aria-hidden="true">
          <span>
            <i style={{ boxShadow: 'inset 0 0 0 1.5px var(--mt-vien-dam)' }} />
            không học
          </span>
          <span>
...
function KhoiLoiAi({ pm }: { pm: PhMoi }) {
  ...
  return (
    <section className="mt-muc" id="loi-ai" aria-labelledby="h-9">
      <div className="mt-muc__dau">
        <h2 id="h-9">Lời A.I Đỗ Đại Học gửi anh/chị</h2>
      </div>
      {l && (
        <div className="mt-thu" data-vung="loi-ai">
      ...
      {goiY.length > 0 && (
        <div className="mt-goi-y" data-vung="goi-y">
...
export default function BangMoiThu({ pm, sbd, lop, giaoThem, onVe, mucDau }: BangMoiThuProps) {
  ...
  return (
    <div className="m3 mt" data-vung="bang-moi-thu" ref={goc}>
      <header className="mt-tren">
        <button type="button" className="mt-nut-tron" onClick={onVe} aria-label="Về màn chính">
          <ArrowLeft className="mt-i" aria-hidden="true" />
        </button>
        <div className="mt-tren-ten">
          <h1>Mọi thứ về con</h1>
          <p>{[pm.hoTen, lop ? `Lớp ${lop}` : ''].filter(Boolean).join(' · ')}</p>
        </div>
      </header>
      <div className="mt-khung">
        <div className="mt-canh">
          <nav className="mt-muc-luc" aria-label="Các mục của bảng">
            <span className="mt-muc-luc__tieu">TRONG BẢNG NÀY</span>
            <ul>
              {muc.map((m) => (
                <li key={m.id}>
