...
// ---------------------------------------------------------------------------
// <NutGat> — nút gạt bật/tắt một tuỳ chọn. Dùng `role="switch"` chứ không phải
// checkbox: hai thứ này đọc màn hình phát âm khác nhau, và thầy gọi đúng tên
// "nút gạt". Cả hàng là vùng chạm (≥48px, phép kiểm hiển thị số 13) — bấm vào
// chữ cũng gạt được, không phải nhắm đúng con trượt.
// ---------------------------------------------------------------------------
export function NutGat({
  bat,
  onDoi,
  ten,
  mota,
  ...
  ten: string
  /** Câu giải thích đổi theo trạng thái — thầy đọc là biết bật/tắt được gì. */
  mota?: ReactNode
  className?: string
}) {
  return (
    <button
      ...
      aria-label={ten}
      onClick={() => onDoi(!bat)}
      className={`tap-target w-full text-left flex items-center ${className}`}
      style={{ gap: 'var(--k3)', minHeight: 48, padding: 'var(--k2) var(--k3)', borderRadius: 'var(--bo-1)', background: 'transparent', border: 'none', color: 'var(--muc)' }}
    >
      <span className="flex-1 min-w-0">
        <span className="block font-bold" style={{ fontFamily: 'var(--sans)', fontSize: 'var(--cx-2)' }}>
          {ten}
        </span>
        {mota && (
          <span className="block" style={{ fontFamily: 'var(--sans)', fontSize: 'var(--cx-1)', color: 'var(--nhat)' }}>
            {mota}
          </span>
        )}
      </span>
      <span
        ...
        style={{
          ...
          padding: 3,
          background: bat ? 'var(--xanh)' : 'var(--vien-dam)',
          justifyContent: bat ? 'flex-end' : 'flex-start',
          transitionProperty: 'background-color',
          transitionDuration: 'var(--nhanh)',
        }}
      >
        <span style={{ width: 24, height: 24, borderRadius: 'var(--bo-tron)', background: 'var(--giay)', boxShadow: 'var(--bong-1)' }} />
      </span>
    </button>
  )
}
