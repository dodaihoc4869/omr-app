...
import { gioHanVietNam } from '../lib/han-bai-tap'
...
export default function BangTroLyHocSinh({
  ...
  const [moRadar, setMoRadar] = useState(true)
  ...
  }, [now, sbd, hoTen, dsBtvn, dsMomGiao, dsLichSu, tongSoCauSai, hoSoThanThu])
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200/70 dark:border-slate-800/80">
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <span
              title="Kế hoạch từ dữ liệu bài tập đã tải"
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Làm bài gần hạn trước, rồi luyện lại câu sai.
            </p>
          </div>
        </div>
      ...
      <div className="rounded-2xl bg-white/90 dark:bg-slate-800/80 border border-slate-200/80 dark:border-slate-750 p-3.5 sm:p-4 shadow-2xs flex items-start gap-3">
        ...
        <div className="space-y-1 text-xs sm:text-sm">
          ...
          <div className="text-[11px] text-purple-700 dark:text-purple-300 font-medium pt-1">
            Thần thú: {thanThu.thongDiepThu}
          </div>
        </div>
      </div>

      {radarDeadline.quaHan > 0 && <p className="rounded-xl bg-amber-50 p-3 text-sm text-amber-900 dark:bg-amber-950 dark:text-amber-200">BTVN quá hạn cần Thầy gia hạn. Bài gia đình giao hết giờ cần mở để nộp phần đã lưu.</p>}
            ...
            return (
              ...
              >
                <div>
                  ...
                  {task.hanNop && <p className="text-xs text-slate-600 dark:text-slate-300 mb-2">Hạn: {gioHanVietNam(task.hanNop)} (giờ Việt Nam)</p>}
                ...
                <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800/60">
                  <div className="flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400 font-medium">
                    <span>Ước tính ~{task.phutUocTinh} phút</span>
                    <span className="text-purple-600 dark:text-purple-400 font-bold">Phần thưởng theo kết quả</span>
                  </div>
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800/60">
                {radarDeadline.danhSach.map((item) => (
                  <button
                    ...
                    className={`w-full text-left py-2 flex items-center justify-between gap-2 text-xs ${
                  ...
                  >
                    <div className="min-w-0 flex-1 truncate font-medium text-slate-800 dark:text-slate-200">
                      {item.tieuDe}
                      {item.hanNop && <span className="block font-normal whitespace-normal">{gioHanVietNam(item.hanNop)} (giờ Việt Nam)</span>}
                    </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-xs flex-wrap gap-2">
          <div className="flex items-center gap-1.5 font-bold text-slate-800 dark:text-slate-200">
            ...
            <span>Số câu đã nộp hôm nay: {nganSach.daLamCau} / {nganSach.mucTieuCau} câu ({nganSach.phanTramHoanThanh}%)</span>
          </div>
          <div className="text-slate-500 dark:text-slate-400">
            {nganSach.phutConLaiUocTinh > 0
              ? `Ước tính còn ~${nganSach.phutConLaiUocTinh} phút học`
              : 'Đã đạt số câu gợi ý. Kiểm tra bài còn phải nộp.'}
          </div>
        </div>
        ...
        <div className="w-full h-3 rounded-full bg-slate-200 dark:bg-slate-800 overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, Math.max(0, nganSach.phanTramHoanThanh))}%` }}
          />
        </div>
