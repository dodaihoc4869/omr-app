...
export default function PhieuScreen({ duCoSan, laCuaEm = false, xinLink }: { duCoSan?: PhieuDayDu; laCuaEm?: boolean; xinLink?: () => Promise<string> } = {}) {
  ...
  const tk = du.thongKe
  const boTrongCount = du.cauSai ? du.cauSai.filter((c) => !String(c.dapAnChon ?? '').trim() || (c.phan === 'II' && /^-{1,4}$/.test(String(c.dapAnChon ?? '').trim()))).length : 0
  const soSaiThat = du.thongKe ? du.thongKe.soSai : (du.cauSai ? du.cauSai.length - boTrongCount : (du.soCauSai ?? 0))
  const dung = du.thongKe ? Math.max(0, du.thongKe.tongCau - du.thongKe.soSai - du.thongKe.soBoTrong) : (du.tongSoCau !== null ? Math.max(0, du.tongSoCau - soSaiThat - boTrongCount) : null)
