...
export function dungThanThu3D(info: ThanThuInfo, cap: number, soLop?: number): BoThanThu3D {
  ...
  for (const ben of [-1, 1] as const) {
    ...
    // z = 1.04: khung này đã nhân `VIEN_LONG`, nên mặt lông nằm ở 1.0. Nhãn
    // cầu bán kính 0.31 nhô nhẹ qua khỏi lông — đặt 1.04 là mắt nhô hẳn ra ngoài
    // không bị lớp lông dày nuốt chửng tròng mắt.
    g.position.set(ben * nm.x, nm.y, 1.04)
    ...
    if (nm.may > 0) {
      ...
      my.position.set(ben * nm.x, nm.y + 0.2 * nm.co, 1.02)
