...
export default function Game({sbd,token:initialToken,manDau,onDong}:Props){
 ...
 return <section className="spirit-game" aria-label="Thần Thú Hoá Học">
  ...
  {profile&&<>
   <nav className="spirit-nav" aria-label="Mục game">{([['home','Đảo thần thú'],['doan','Đoàn Hộ Tống'],['arena','Võ đài thứ Bảy'],['progress','Tiến bộ của em'],['coming','Game mới · Sắp ra mắt']] as const).map(([id,label])=><button key={id} aria-current={tab===id?'page':undefined} onClick={()=>setTab(id)}>{label}</button>)}</nav>
   {!profile.choice&&<>
   ...
   {profile.choice?<div className="spirit-panel">{tab==='doan'&&<p className="spirit-moi-doan"><strong>Đoàn Hộ Tống đang chờ em.</strong> Em chọn một thần thú để cùng cả lớp hộ tống Linh Tâm — chọn xong là lên đường được ngay.</p>}<h2>Chọn bạn đồng hành</h2><p>Cả tám thần thú đều học được mọi phần Hoá. Cấp và EXP cũ được giữ khi chuyển hình dạng.</p><div className="spirit-pets">{PETS.map((p,i)=><button key={p.id} disabled={busy} onClick={()=>void run(()=>request('choose',{pet:p.id}))}><SpiritArt index={i}/><strong>{p.name}</strong><span>{p.element}</span><small>{p.skill}</small></button>)}</div></div>:<>
   {tab==='home'&&<>
