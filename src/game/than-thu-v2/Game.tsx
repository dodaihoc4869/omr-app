...
export default function Game({sbd,token:initialToken,onDong}:Props){
 ...
 const [profile,setProfile]=useState<Profile|null>(null);const [,setRevision]=useState(0);const [tab,setTab]=useState<'home'|'learn'|'arena'|'progress'|'coming'>('home')
 ...
 const request=useCallback(async(action:string,data:Record<string,unknown>={},sessionToken=token):Promise<Result>=>{
  ...
  if(mounted.current){if(r.tasks)setTasks(r.tasks);if(r.profile&&(r.revision??0)>=latestRevision.current){setProfile(r.profile);latestRevision.current=r.revision??0;if(r.revision!==undefined)setRevision(r.revision)}}return r
 },[token,sbd])
 ...
 return <section className="spirit-game" aria-label="Thần Thú Hoá Học">
  ...
  {profile&&<>
   <nav className="spirit-nav" aria-label="Mục game">{([['home','Đảo thần thú'],['arena','Hộ Tống Linh Tâm'],['progress','Tiến bộ của em'],['coming','Game mới · Sắp ra mắt']] as const).map(([id,label])=><button key={id} aria-current={tab===id?'page':undefined} onClick={()=>setTab(id)}>{label}</button>)}</nav>
   {!profile.choice&&<>
   ...
   <div className="spirit-panel mission-exp"><small>EXP TỪ BÀI HỌC</small><strong>{profile.academic?.today??0}<span> / {profile.academic?.dailyLimit??100}</span></strong><small>hôm nay · tự cộng vào thần thú</small>{expPending>0&&<small>{expPending} bài chờ đối chiếu</small>}<details><summary>Cách nhận EXP</summary><p>2 EXP/câu đúng; bài kiểm tra thêm tối đa 10 EXP theo điểm. Câu đã nhận quà không cộng lại.</p></details></div>
   <ImmortalShield level={profile.cap} state={profile.shields} busy={busy} onUse={id=>run(()=>request('shield-use',{useId:id}))}/></div>
   <div className="spirit-panel mission-suggestion"><small>NHIỆM VỤ DÀNH CHO EM</small><h2>Luyện đúng phần cần tiến bộ</h2><p>Ưu tiên câu cần củng cố, kiến thức đến hạn ôn và câu mới vừa sức từ phần em đã học.</p>
   {recommendation?.suggestions?.length===0&&<p>{recommendation.remaining===0?'Em đã hoàn thành 200 câu hôm nay. Ngày mai sẽ có nhiệm vụ mới.':'Chưa có câu phù hợp từ phần đã học. Em hoàn thành bài Thầy giao rồi quay lại nhé.'}</p>}<div className="mission-topics">{recommendation?.suggestions?.slice(0,3).map((x,i)=><div key={i}><strong>{x.title}</strong><small>{x.source} · {x.part==='I'?'Trắc nghiệm':x.part==='II'?'Đúng / sai':'Trả lời ngắn'}</small></div>)}</div>
   <button className="spirit-start-button" disabled={busy||recommendation?.remaining===0} onClick={()=>void start('adventure')}>Bắt đầu làm nhiệm vụ mới →</button><small>{recommendation?`${recommendation.dailyUsed??0} / 200 câu hôm nay · mỗi nhiệm vụ tối đa 6 câu`:'Tối đa 200 câu mỗi ngày · mỗi nhiệm vụ tối đa 6 câu'}</small><small>Làm bài để nhận EXP theo tiến bộ; câu lặp lại không nhận thưởng thêm.</small></div></div>}
   {profile.choice?<div className="spirit-panel"><h2>Chọn bạn đồng hành</h2><p>Cả tám thần thú đều học được mọi phần Hoá. Cấp và EXP cũ được giữ khi chuyển hình dạng.</p><div className="spirit-pets">{PETS.map((p,i)=><button key={p.id} disabled={busy} onClick={()=>void run(()=>request('choose',{pet:p.id}))}><SpiritArt index={i}/><strong>{p.name}</strong><span>{p.element}</span><small>{p.skill}</small></button>)}</div></div>:<>
   {tab==='home'&&<>
   ...
   <div hidden={tab!=='arena'}><EscortRoom storageKey={sbd} call={request} active={tab==='arena'}/></div>
