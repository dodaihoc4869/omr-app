...
export default function ImmortalShield({level,state,busy,onUse,serverRemaining,ngayDat,ngayMo}:{level:number;serverRemaining?:number;ngayDat?:number;ngayMo?:number;state?:ShieldState;busy:boolean;onUse:(id:string)=>Promise<unknown>}){
 ...
 useEffect(()=>{const earned=shieldEntitlement(level)-shieldEntitlement(previousLevel.current);if(earned>0&&serverRemaining===undefined)setGift(earned);previousLevel.current=level},[level,serverRemaining])
 const until=state?.activeUntil??0,active=until>now,remaining=serverRemaining??shieldRemaining(level,state),stage=evolutionStage(level),tier=Math.max(0,stage-1)
