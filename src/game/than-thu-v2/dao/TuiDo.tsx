...
export default function TuiDo({profile,exp,busy=false,loi='',onDungKhien,onMoVoDai,onMoTienBo}:TuiDoProps){
 ...
 return <div className="dao-tui">
  ...
  <section className="dao-kinh dao-tui-the" aria-label="Khiên chống đuổi"><ImmortalShield level={profile.cap} state={profile.shields} busy={busy} onUse={onDungKhien}/></section>
  {k&&<section className="dao-kinh dao-tui-the" aria-label="Mảnh khiên"><div className="dao-tui-dong"><span className="dao-tui-so">{k.manh%k.moiKhien}</span><div><strong>Mảnh khiên · {k.manh%k.moiKhien}/{k.moiKhien}</strong><p>Làm bài đều mỗi ngày để nhận mảnh. Đủ <b>{k.moiKhien} mảnh</b> là rèn được một khiên chống đuổi{typeof daRen==='number'&&daRen>0?<> · em đã rèn <b>{daRen}</b> khiên</>:null}.</p></div></div>
   <div className="dao-thanh" role="progressbar" aria-label="Tiến độ rèn khiên" aria-valuemin={0} aria-valuemax={k.moiKhien} aria-valuenow={k.manh%k.moiKhien}><i style={{width:`${Math.round(k.manh%k.moiKhien/k.moiKhien*100)}%`}}/></div></section>}
