...
| 6 | vitest: tệp mới xanh, MỚI ĐỎ = [] | 7 tệp `tests/dao-*` 59/59 xanh; bộ hợp đồng game (than-thu, learning-battle, evolution-v2, kenh4, escort-*, doan-*) 42 tệp: đỏ 8 tệp `than-thu-*` của game CŨ, cả 8 đều có trong `docs/nen-vitest-do-1909.txt`, không tệp nào nạp mã của đảo | log so tên ngày 21/09 |
...
VIỆC CÒN LẠI (không phải của Code 6 hoặc chờ phiên khác): (a) Code 5 nối `Game.tsx` theo hợp đồng prop rồi chạy Playwright trên bản nối thật; (b) khi `ly-do-thuong.ts` của Code 5 lên main: đổi `lyDoThuongTuKetQua` trong `dao-core.ts` sang import (câu chữ đang trùng khớp); (c) cổng HS (Code 2) đưa `exp` + chuỗi ngày xuống `Game` nếu muốn hiện chip — vắng thì đảo tự ẩn; (d) bảng TÊN chương cho Sổ tay (kho chỉ có mã) — có thì truyền `tenChuong`.
