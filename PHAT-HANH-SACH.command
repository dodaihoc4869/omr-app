...
git archive HEAD | tar -x -C "$TMP" || { echo ">>> Không trích được HEAD."; exit 1; }
ln -s "$GOC/node_modules" "$TMP/node_modules"

( cd "$TMP" && unset GITHUB_PAGES && npm run build ) \
  || { echo ">>> DỰNG HỎNG — đọc lỗi ở trên. Bản đang chạy KHÔNG bị đụng."; exit 1; }
...
if [ $MA -eq 0 ]; then
  ...
  echo "    Bản vừa phát hành : $MA_HEAD"
