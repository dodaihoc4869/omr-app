...
Mã lỗi: `duoi_nguong` · `thieu_vang` · `chua_mo` · `het_suat` · `da_co` · `gia_doi` · `tam_dong` · `chua_co` · `khong_co_mon` (mã món không có) · `sap_mo` (món đợt 2) · `sai_dau_vao`. **[chốt]** `khong_co_mon`, `sap_mo`, `sai_dau_vao` là mã mới, lời do Boss viết 21/09: "Cửa hàng không có món này. Em tải lại Cửa hàng rồi chọn lại nhé." · "Món này sắp mở bán. Em ghé lại sau nhé." · "Có gì đó chưa đúng. Em tải lại trang rồi thử lại nhé." Hai lời tạm CHỜ Boss duyệt: `chua_mo` khi thiếu ấn thạch — "Món này cần {5} ấn thạch sáng. Em đang có {3}." (chuỗi ngày dùng L3 nguyên văn) · lỗi bận sau 3 lần thử đổi vàng — "Đổi vàng chưa xong vì hồ sơ đang bận. Em bấm lại nhé, số EXP của em chưa bị trừ." (không có `ma`, là lỗi thường của game).

## 4. Năm lệnh
1. `vang-xem` — vào `{}` · ra `{"ok":true,"bat":true,"vang":340,"ongNghiem":620,"giuLai":200,"doiToiDa":420,"ngayAn":3,"chuoiNgay":9,"anThachSang":3,"mua":"m1"}`. `ongNghiem` = `wallet` của hồ sơ game (EXP chưa nạp); `giuLai` = 200 (hằng, luật hấp thụ); `doiToiDa = max(0, ongNghiem − 200)`; `ngayAn = floor(ongNghiem / 200)`; `vang = SUM(vang_so.so_vang)` của em; `chuoiNgay` = số ngày liên tiếp ĐẠT nhiệm vụ ngày (cùng định nghĩa `demChuoiDat`: ngày nghỉ không đứt, `mot_phan`/`khong` đứt), tính cả hôm nay nếu đã đạt. **[chốt] LƯU Ý:** `chuoiDat` của kế hoạch ngày chỉ nhìn lại 7 ngày (`SO_NGAY_LICH_SU = 7`) nên Bảng nhiệm vụ không bao giờ hiện quá 7-8; điều kiện "chuỗi 14–30 ngày" sẽ không đạt được nếu đọc từ đó. Cửa hàng đọc CÙNG định nghĩa với cửa sổ 45 ngày (`SO_NGAY_CHUOI_SHOP`); số hiện ở cửa hàng có thể lớn hơn số ở Bảng nhiệm vụ khi chuỗi > 7; `anThachSang` = số ấn thạch ở trạng thái sáng (`docAnThach`).
...
- [x] S2 `vang-xem`, `vang-doi`, `shop-danh-sach`, `shop-mua`, `thu-mac-do` (server/src/game-v2-shop.ts, nối ở game-v2.ts) + test 34 (tests/shop-phu-kien-may-chu-2109.test.ts: bấm đúp, đua song song và đua TẤT ĐỊNH, không âm, giữ ≥ 200, hết cái cuối, giaThay). Lỗ hổng của bản phác (dòng sổ dựa vào "hồ sơ ở revision+1") đã thay bằng ghi sổ trước + khoá theo revision đã đọc.
