...
## 11 · LƯỢT CHIỀU vs LƯỢT ĐÊM — cùng khoá `(sbd, ngay)` (Code 3, 21/09, Boss + Code 1 báo)
- `ai_dieu_chinh` có khoá `(sbd, ngay)`: hàng chiều (chỉ `thuThach` + `loiMoi`, núm rỗng) và hàng đêm của CÙNG NGÀY là MỘT hàng. Trước bản sửa, `ON CONFLICT` ghi đè cả `json` ⇒ chiều xoá núm của đêm; và hàng chiều ở ngày mới hơn thì "đè" hàng đêm còn hạn 3 ngày khi tầng đọc lấy hàng mới nhất.
- **Nhận diện bằng NỘI DUNG (không migration, `laPhanTuChieu` ở `bo-nao.ts`):** có `thuThach` VÀ nhịp 0 VÀ `dang` rỗng VÀ `khacPhuc` rỗng VÀ không lời nào (`loiNhanChoEm`, `loiNhanChoPhuHuynh`, `thuTuan`). `nop.mjs --chieu` của Code 1 KHÔNG cần đổi.
- **Ghi:** (1) đã có hàng đêm cùng ngày ⇒ chỉ GỘP `thuThach` + `loiMoi` vào `json` (núm, `che_do`, `ap_dung`, `do_tin`, `het_han` giữ nguyên; không tính là điều chỉnh mới: `nhan` không tăng, `nhanChieu` +1). (2) Chưa có hàng đêm ⇒ tạo hàng mới mang dấu `json.luot = "chieu"` (hàng chiều-riêng-lẻ). (3) Đêm nộp lại SAU chiều ⇒ giữ `thuThach` + `loiMoi` đã có.
