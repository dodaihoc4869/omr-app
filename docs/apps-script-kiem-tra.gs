...
// BÀI TẬP TỰ LUYỆN (LINK-BAI-LUYEN.md v2) — TÁCH HẲN khỏi ca thi.
//
// Thầy copy link ở hồ sơ em, gửi Zalo; em mở link, làm, nộp; máy chủ chấm và
// ghi kết quả VÀO ĐÚNG SHEET NÀY, không đi đâu khác.
//
// SỬA 05/09 (thầy chốt sau khi đọc bản đầu): bài tự luyện KHÔNG cộng dồn vào
// bảng mạnh–yếu nữa. Lý do đứng vững: em làm ở nhà, được mở sách vở tra cứu,
// nên tỉ lệ sai ở đây không cùng thang đo với bài làm trong lớp có người coi.
// Trộn hai thang vào một bảng là bảng mạnh–yếu nói dối thầy.
//
// Vậy nên nó KHÔNG đụng TienDoCa, TienDoHS, QidDaLam, LuotThi, CaKiemTra,
// ChiTietCau, điểm số, xếp hạng — MỘT SHEET DUY NHẤT. Số liệu đi vào báo cáo
// phụ huynh bằng một khối biểu đồ riêng, tách khỏi phần mạnh–yếu của ca thi.
//
// ChiTietJson NHẸ: mỗi câu chỉ {qid, phan, chuyenDe, mucDo, chon, dung}. Bộ câu
// đầy đủ (có nội dung, có ảnh) nằm ở Drive như ca_<mã>_bank.json — Sheet chỉ
// giữ tham chiếu, vì một ô Sheet chỉ chứa được 50.000 ký tự.
const SHEET_TULUYEN = 'BaiTuLuyen'
const TULUYEN_HEADERS = ['Ma', 'SBD', 'HoTen', 'TaoLuc', 'SoCau', 'ChuyenDeJson', 'HanXem', 'LanThu', 'NopLuc', 'SoDung', 'SoCauLam', 'ChiTietJson', 'IdThietBi', 'GhiChu']
// Mã sai định dạng và mã không tồn tại phải trả về CÙNG MỘT CÂU — không để ai
// dò ra quy tắc mã qua thông điệp lỗi (đúng luật đã chốt cho layPhieu).
const TULUYEN_LOI = 'Không tìm thấy bài luyện'
const TULUYEN_RE_MA = /^[A-Za-z0-9_-]{8,40}$/
// Năm khoá phải gỡ sạch trước khi gửi đề xuống máy em. Em mở công cụ nhà phát
// triển là thấy phản hồi mạng; còn một khoá là lộ đáp án trước khi làm, và dữ
// liệu mạnh–yếu thu về thành số rác đi thẳng vào phiếu gửi phụ huynh.
const TULUYEN_KHOA_BI_MAT = ['dapAn', 'chot', 'lyDo', 'buoc', 'ketQua']
// Số buổi tự luyện gần nhất gửi kèm xuống máy em để vẽ biểu đồ đầu trang.
const TULUYEN_SO_BUOI_GUI = 20

function sheetTuLuyen_() {
  const sh = getSheet_(SHEET_TULUYEN, TULUYEN_HEADERS)
  boSungTieuDe_(sh, TULUYEN_HEADERS)
  return sh
}

/** Gỡ đáp án + lời giải khỏi bộ câu. Thuần hàm, test đọc thẳng được. */
function goDapAnTuLuyen_(cau) {
  const ra = []
  for (let i = 0; i < (cau || []).length; i++) {
    ...
    for (let j = 0; j < ks.length; j++) {
      if (TULUYEN_KHOA_BI_MAT.indexOf(ks[j]) >= 0) continue
      o[ks[j]] = c[ks[j]]
    }
    ra.push(o)
  }
...
/** Các buổi tự luyện TRƯỚC của một em, đủ nhẹ để gửi xuống máy em vẽ biểu đồ.
 *
 * Chỉ trả bài ĐÃ NỘP, bỏ chính bài đang mở, và mỗi bài chỉ lấy LẦN LÀM CUỐI —
 * một bài làm lại năm lượt mà đếm cả năm thì đường tiến bộ nói dối. Trần
 * TULUYEN_SO_BUOI_GUI buổi gần nhất: gói phải qua được đường mạng của em.
 *
 * KHÔNG có nội dung câu, KHÔNG có đáp án — chỉ qid, phần, chuyên đề, mức độ,
 * đúng hay sai. */
function lichSuEmTuLuyen_(sh, sbd, boQuaMa) {
  const data = sh.getDataRange().getValues()
  const ra = []
  for (let i = 1; i < data.length; i++) {
    const v = data[i]
    if (!v[0] || String(v[1]) !== String(sbd)) continue
    if (String(v[0]) === String(boQuaMa)) continue
    if (!v[8]) continue
    let lichSu = []
    try { lichSu = v[11] ? JSON.parse(v[11]) : [] } catch (err) { lichSu = [] }
    if (!lichSu.length || !lichSu[lichSu.length - 1].lanThu) continue
    const cuoi = lichSu[lichSu.length - 1]
    ra.push({
      ma: String(v[0]),
      nopLuc: String(v[8]),
      soCau: Number(cuoi.soCau) || 0,
      soDung: Number(cuoi.soDung) || 0,
      lanThu: Number(cuoi.lanThu) || 1,
      cau: cuoi.cau || [],
    })
  }
  ra.sort(function (a, b) { return msCua_(a.nopLuc) - msCua_(b.nopLuc) })
  return ra.length > TULUYEN_SO_BUOI_GUI ? ra.slice(ra.length - TULUYEN_SO_BUOI_GUI) : ra
}

/** Cửa sổ N câu bắt đầu từ `batDau`, QUAY VÒNG khi chạm cuối gói.
 *
 * `soCau` <= 0 hoặc lớn hơn gói ⇒ lấy trọn gói. Quay vòng chứ không cắt cụt: em
 * làm tới lần thứ năm mà gói chỉ đủ bốn cửa sổ thì thà gặp lại đề cũ, còn hơn
 * nhận một bài rỗng. */
function cuaCauTuLuyen_(cau, batDau, soCau) {
  const n = (cau || []).length
  if (!n) return []
  if (!soCau || soCau <= 0 || soCau >= n) return cau
  const ra = []
  const d = ((Number(batDau) || 0) % n + n) % n
  for (let i = 0; i < soCau; i++) ra.push(cau[(d + i) % n])
  return ra
}

/** Phần III: bỏ khoảng trắng hai đầu, dấu phẩy thập phân thành dấu chấm. */
function chuanSoTuLuyen_(x) {
  return String(x === null || x === undefined ? '' : x).trim().replace(',', '.')
}

/** Một câu đúng hay sai. Luật TRÙNG KHỚP `taoChiTietCau` của ca thi, không nới
 * cũng không siết: cùng một hiểu biết mà hai đường cho ra hai tỉ lệ sai khác
 * nhau thì bảng mạnh–yếu vô nghĩa.
 *   I   — phải chọn, và chọn đúng chữ.
 *   II  — đúng khi CẢ BỐN Ý khớp.
 *   III — bỏ trống là sai; so sau khi chuẩn hoá số. */
function cauDungTuLuyen_(phan, chon, dapAn) {
  const c = String(chon === null || chon === undefined ? '' : chon)
  const d = String(dapAn === null || dapAn === undefined ? '' : dapAn)
  if (phan === 'I') return c !== '' && c === d
  if (phan === 'II') return c === d
  return chuanSoTuLuyen_(c) !== '' && chuanSoTuLuyen_(c) === chuanSoTuLuyen_(d)
}

/** Chấm cả bài. Thuần hàm — đây là chỗ test tự động đối chứng tay-tính lại. */
function chamTuLuyen_(cau, chon) {
  const ra = []
  const ch = chon || {}
  for (let i = 0; i < (cau || []).length; i++) {
    const c = cau[i]
    const x = String(ch[c.id] === null || ch[c.id] === undefined ? '' : ch[c.id])
    ra.push({
      qid: String(c.id),
      phan: c.phan,
      ...
      dung: cauDungTuLuyen_(c.phan, x, String(c.dapAn || '')),
    })
  }
  return ra
}
...
function doPost(e) {
  ...
  // ------------------------------------------------------ BÀI TẬP TỰ LUYỆN
  // Ba lệnh (LINK-BAI-LUYEN.md mục 2.2). `taoTuLuyen` đòi mã bí mật vì chỉ máy
  // thầy được ghi; hai lệnh kia KHÔNG đòi — em chỉ có đường link, đúng như
  // `layPhieu` của phụ huynh. Bù lại chúng bị bó chặt: phải đưa đúng mã, trả về
  // ĐÚNG MỘT bài, không có lệnh nào liệt kê mã.

  if (action === 'taoTuLuyen') {
    const loiTL = kiemTraMaBiMat_(body)
    if (loiTL) return jsonResponse_({ ok: false, error: loiTL })
    const maTL = String(body.ma || '').trim()
    if (!TULUYEN_RE_MA.test(maTL)) return jsonResponse_({ ok: false, error: 'Mã bài luyện không hợp lệ' })
    const sbdTL = String(body.sbd || '').trim()
    if (!sbdTL) return jsonResponse_({ ok: false, error: 'Thiếu số báo danh' })
    const cauTL = body.cau || []
    if (!cauTL.length) return jsonResponse_({ ok: false, error: 'Bộ câu rỗng' })
    const shTL = sheetTuLuyen_()
    const rowTL = findRowByKey_(shTL, 0, maTL)
    const cuTL = rowTL > 0 ? String(shTL.getRange(rowTL, 6).getValue()) : ''
    // Bộ câu ĐẦY ĐỦ (có đáp án, có lời giải, có ảnh) chỉ nằm ở Drive.
    const refTL = luuJsonLon_('tuluyen_' + maTL, { v: 1, sbd: sbdTL, hoTen: String(body.hoTen || ''), taoLuc: new Date().toISOString(), cau: cauTL }, cuTL)
    const cdTL = {}
    for (let i = 0; i < cauTL.length; i++) {
      const cd = String(cauTL[i].chuyenDe || '').trim() || '(chưa phân loại)'
      cdTL[cd] = (cdTL[cd] || 0) + 1
    }
    const hangTL = [maTL, sbdTL, String(body.hoTen || ''), new Date().toISOString(), cauTL.length, JSON.stringify(cdTL), '', 0, '', '', '', '', '', '']
    if (rowTL > 0) {
      hangTL[5] = refTL
      shTL.getRange(rowTL, 1, 1, TULUYEN_HEADERS.length).setValues([hangTL])
    } else {
      hangTL[5] = refTL
      shTL.appendRow(hangTL)
    }
    return jsonResponse_({ ok: true, ma: maTL, soCau: cauTL.length })
  }

  if (action === 'layTuLuyen' || action === 'nopTuLuyen') {
    const maTL = String(body.ma || '').trim()
    if (!TULUYEN_RE_MA.test(maTL)) return jsonResponse_({ ok: false, error: TULUYEN_LOI })
    const shTL = sheetTuLuyen_()
    const rowTL = findRowByKey_(shTL, 0, maTL)
    if (rowTL < 0) return jsonResponse_({ ok: false, error: TULUYEN_LOI })
    const vTL = shTL.getRange(rowTL, 1, 1, TULUYEN_HEADERS.length).getValues()[0]
    const goiTL = docJsonLon_(vTL[5])
    if (!goiTL || !goiTL.cau) return jsonResponse_({ ok: false, error: TULUYEN_LOI })

    if (action === 'layTuLuyen') {
      // ĐỀ ĐÃ GỠ SẠCH đáp án và lời giải. Máy chủ giữ bản đầy đủ, chấm khi em
      // nộp, rồi mới trả lời giải về (mục 2.3).
      //
      // Kèm LỊCH SỬ CÁC BUỔI TRƯỚC của chính em này (thầy chốt 05/09): đầu
      // trang bài luyện luôn có biểu đồ tổng hợp mạnh–yếu và cách làm bài của
      // mọi buổi trước. Chỉ số liệu, KHÔNG nội dung câu, KHÔNG đáp án. Lộ được
      // đúng cho người đã có mã 10 ký tự của em — mà mã đó gắn sẵn SBD và đầu
      // bài đã in tên em, nên không thêm gì mới cho người cầm link.
      return jsonResponse_({
        ok: true,
        ma: maTL,
        sbd: String(vTL[1]),
        hoTen: String(vTL[2] || ''),
        soCau: Number(vTL[4]) || goiTL.cau.length,
        lanThu: Number(vTL[7]) || 0,
        cau: goDapAnTuLuyen_(goiTL.cau),
        lichSuEm: lichSuEmTuLuyen_(shTL, String(vTL[1]), maTL),
        serverNow: Date.now(),
      })
    }

    // --- NỘP: máy chủ chấm ĐÚNG CỬA SỔ câu em được giao lần này.
    //
    // Gói trên Drive chở tới 40 câu. Mỗi lần giao chỉ lấy một cửa sổ N câu, và
    // lần sau dịch sang cửa sổ kế tiếp — giao lần 2 mà lại đúng đề cũ thì em
    // chép lại đáp án lần 1, số liệu thành rác.
    const boChamTL = cuaCauTuLuyen_(goiTL.cau, Number(body.batDau) || 0, Number(body.soCau) || 0)
    const chamTL = chamTuLuyen_(boChamTL, body.chon || {})
    let soDungTL = 0
    for (let i = 0; i < chamTL.length; i++) if (chamTL[i].dung) soDungTL++
    const lanTruocTL = Number(vTL[7]) || 0
    const lanThuTL = lanTruocTL + 1
    const nopLucTL = new Date().toISOString()
    const nheTL = []
    for (let i = 0; i < chamTL.length; i++) {
      nheTL.push({ qid: chamTL[i].qid, phan: chamTL[i].phan, chuyenDe: chamTL[i].chuyenDe, mucDo: chamTL[i].mucDo, chon: chamTL[i].chon, dung: chamTL[i].dung })
    }
    // Cột 8..13: LanThu, NopLuc, SoDung, SoCauLam, ChiTietJson, IdThietBi.
    // ChiTietJson giữ CẢ LỊCH SỬ các lần làm, không ghi đè: biểu đồ tiến bộ
    // trong báo cáo phụ huynh cần thấy lần 1 khác lần 2 thế nào.
    let lichSuTL = []
    try { lichSuTL = vTL[11] ? JSON.parse(vTL[11]) : [] } catch (err) { lichSuTL = [] }
    if (!lichSuTL.length || !lichSuTL[0] || !lichSuTL[0].lanThu) lichSuTL = []
    lichSuTL.push({ lanThu: lanThuTL, nopLuc: nopLucTL, soDung: soDungTL, soCau: chamTL.length, cau: nheTL })
    // Giữ tối đa 10 lần gần nhất — ô Sheet chỉ chứa 50.000 ký tự.
    if (lichSuTL.length > 10) lichSuTL = lichSuTL.slice(lichSuTL.length - 10)
    shTL.getRange(rowTL, 8, 1, 6).setValues([[lanThuTL, nopLucTL, soDungTL, chamTL.length, JSON.stringify(lichSuTL), String(body.idThietBi || '')]])
  ...
  if (action === 'diemTuLuyen') {
    // Trang BÁO CÁO PHỤ HUYNH đọc để bật nút "Xem điểm của con" và để biết đã
    // giao tới lần thứ mấy. KHÔNG đòi mã bí mật (trang đó không có), nhưng phải
    // biết đúng mã 10 ký tự của bài — cùng luật với `layPhieu`.
    //
    // Chỉ trả CON SỐ: không câu hỏi, không đáp án, không lời giải.
    const maDL = String(body.ma || '').trim()
    if (!TULUYEN_RE_MA.test(maDL)) return jsonResponse_({ ok: false, error: TULUYEN_LOI })
    const shDL = sheetTuLuyen_()
    const rowDL = findRowByKey_(shDL, 0, maDL)
    if (rowDL < 0) return jsonResponse_({ ok: false, error: TULUYEN_LOI })
    const vDL = shDL.getRange(rowDL, 1, 1, TULUYEN_HEADERS.length).getValues()[0]
    let lichSuDL = []
    try { lichSuDL = vDL[11] ? JSON.parse(vDL[11]) : [] } catch (err) { lichSuDL = [] }
    if (lichSuDL.length && !lichSuDL[0].lanThu) lichSuDL = []
    const lan = []
    for (let i = 0; i < lichSuDL.length; i++) {
      const l = lichSuDL[i]
      const theoCd = {}
      for (let j = 0; j < (l.cau || []).length; j++) {
        const c = l.cau[j]
        const cd = String(c.chuyenDe || '').trim() || '(chưa phân loại)'
        if (!theoCd[cd]) theoCd[cd] = { soCau: 0, soDung: 0 }
        theoCd[cd].soCau++
        if (c.dung) theoCd[cd].soDung++
      }
      lan.push({ lanThu: l.lanThu, nopLuc: l.nopLuc, soCau: l.soCau, soDung: l.soDung, theoChuyenDe: theoCd })
    }
    return jsonResponse_({
      ok: true,
      ma: maDL,
      hoTen: String(vDL[2] || ''),
      tongCauTrongGoi: Number(vDL[4]) || 0,
      lanThu: Number(vDL[7]) || 0,
      nopLuc: vDL[8] ? String(vDL[8]) : '',
      lan: lan,
      serverNow: Date.now(),
    })
  }

  if (action === 'danhSachTuLuyen') {
    // Máy thầy đọc để dựng khối biểu đồ trong báo cáo phụ huynh, và để dựng màn
    // Bài tự luyện. Đòi mã bí mật: đây là lệnh LIỆT KÊ, khác hẳn `layTuLuyen`
    // vốn chỉ trả đúng một bài khi biết mã.
    const loiDS = kiemTraMaBiMat_(body)
    if (loiDS) return jsonResponse_({ ok: false, error: loiDS })
    const shDS = sheetTuLuyen_()
    const dataDS = shDS.getDataRange().getValues()
    const locSbd = String(body.sbd || '').trim()
    const itemsDS = []
    for (let i = 1; i < dataDS.length; i++) {
      const v = dataDS[i]
      if (!v[0]) continue
      if (locSbd && String(v[1]) !== locSbd) continue
      let lichSu = []
      try { lichSu = v[11] ? JSON.parse(v[11]) : [] } catch (err) { lichSu = [] }
      if (lichSu.length && !lichSu[0].lanThu) lichSu = []
      let theoCd = {}
      try { theoCd = v[5] ? JSON.parse(v[5]) : {} } catch (err) { theoCd = {} }
      itemsDS.push({
        ma: String(v[0]),
        sbd: String(v[1]),
        hoTen: String(v[2] || ''),
        taoLuc: v[3] ? String(v[3]) : '',
        soCau: Number(v[4]) || 0,
        chuyenDe: theoCd,
        lanThu: Number(v[7]) || 0,
        nopLuc: v[8] ? String(v[8]) : '',
        soDung: v[9] === '' || v[9] === null ? null : Number(v[9]),
        soCauLam: v[10] === '' || v[10] === null ? null : Number(v[10]),
        lichSu: body.canLichSu ? lichSu : [],
      })
    }
    itemsDS.sort(function (a, b) { return msCua_(b.taoLuc) - msCua_(a.taoLuc) })
    return jsonResponse_({ ok: true, items: itemsDS, serverNow: Date.now() })
  }

  if (action === 'xoaTuLuyen') {
    const loiXT = kiemTraMaBiMat_(body)
    if (loiXT) return jsonResponse_({ ok: false, error: loiXT })
    const shXT = sheetTuLuyen_()
    const rowXT = findRowByKey_(shXT, 0, String(body.ma || '').trim())
    if (rowXT > 0) {
      const refXT = String(shXT.getRange(rowXT, 6).getValue())
      if (refXT.indexOf('drive:') === 0) {
        try { DriveApp.getFileById(refXT.slice(6)).setTrashed(true) } catch (err) {}
      }
      shXT.deleteRow(rowXT)
    }
