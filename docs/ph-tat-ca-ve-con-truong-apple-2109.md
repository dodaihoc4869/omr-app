...
| `homNay.tongQuan.viecXong` + `viecTong` | Việc của kế hoạch ngày: `viecTong` = việc còn lại (bắt buộc / nên làm; KHÔNG tính nhãn `tuy_chon`, KHÔNG tính lượt luyện dạng `than_thu`) + `viecXong`. `viecXong` = việc ĐO ĐƯỢC hôm nay từ mốc: ôn lại đã ngồi (kế hoạch không còn việc ôn), bài về nhà đã nộp hôm nay, bài gia đình giao đã nộp hôm nay. Chặng lẻ của bài nâng đỡ CHƯA đếm (sổ không tách chặng chắc chắn). Vắng cả hai khi không có kế hoạch. |
