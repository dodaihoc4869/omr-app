...
4. `POST /game-v2/so-tay` (ĐỌC-CHỈ, không ghi gì) → `{ ok, dang: [{ key, ten, chuong }] }`: mọi dạng em ĐƯỢC PHÉP làm — đúng phạm vi của `start` (readScope + allowed + đề bảo vệ + phạm vi thầy đặt). `key` = `q.dang ?? q.group` (khớp `mastery[].key`), `ten` = tenDang, `chuong` = mã chương (phần trước dấu chấm của mã dạng, vd `ES`; kho không có TÊN chương) hoặc `''`. Dạng chưa có bằng chứng thì KHÔNG có trong danh sách (không lộ tên).
