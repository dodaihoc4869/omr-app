...
- **ĐÃ DỌN (Code 4, 21/09 14:4x):** **G5** — 10 câu báo lỗi "vào Ngân hàng câu hỏi → Cấu hình" ⇒ "vào Cài đặt → Kết nối máy chủ" ở `HocSinhScreen` (2), `LichSuCaScreen` (2), `ExamMonitorScreen` (2, chỉ chữ), `CauHoiScreen` (1), `NutDongBoMoiCa` (2), `lib/exam-api.ts` (1); test khoá `g5-chi-duong-cai-dat-2109`. Chưa đổi: `GoiLenBangScreen:362` (Code 1, đã nhắn), `KhoiTroLy` + `lib/tro-ly/tra-loi.ts` (mã của nhắn tin cũ D3; tra-loi.ts còn được cổng HS/PH nhập — để Code 2).
- **ĐÃ DỌN (Code 4, 21/09 14:5x):** **G10** — tab/nút mục lịch sử ở màn Học sinh còn MỘT tên "Lịch sử ca kiểm tra" (bỏ "Mức độ tiến bộ" ở tab hồ sơ và ở nút danh sách; số ca đi kèm); test khoá trong `hoc-sinh-m3-2109`.
- **ĐÃ DỌN (Code 4, 21/09 15:3x):** **D3** (phần component) — xoá `MessagesFab.tsx` (505 dòng), `KhoiTroLy.tsx` (216), `BongBongChatHocSinh.tsx` (755), `BongBongChatPhuHuynh.tsx` (307): 0 nơi nhập trong `src/` (chỉ đọc-chuỗi trong test). Sửa 3 test từng khoá các tệp này: bỏ khối `Bong bóng nổi dựng đúng cách` (tro-ly.test.ts), khối `Bong bóng hỏi lại đúng cách` (dem-tin-moi.test.ts; phần đếm tin chưa đọc phía Apps Script giữ nguyên), 2 chỗ nhắc MessagesFab ở tu-ngu-app-thay-2109; `mat-khau-ca-va-bao-cao` (kiểm ExamTakeScreen/StudentPortalScreen KHÔNG chứa BongBongChatHocSinh) vẫn đúng, giữ. `tsc -b` + `tsc -p server` sạch, check:mau 0, 6 tệp test dính xanh (77). Không có CSS riêng để dọn (chỉ lớp Tailwind).
- **Mã chết MỚI LỘ RA sau D3 (chưa xoá, chờ Boss):** cả 5 tệp `src/lib/tro-ly/{nguon,y-dinh,tra-loi,ai-giai-bai,he-thong-chat}.ts` nay 0 nơi nhập trong `src/` (chỉ còn test: tro-ly, ai-giai-bai-1309, he-thong-chat-thong-nhau); trong `lib/exam-api.ts` các hàm `listParentMessages`, `markMessagesRead`, `demTinMoi` và kiểu `ParentMessage` chỉ còn `tro-ly/nguon.ts` nhập (lệnh Apps Script `demTinMoi`/`listMessages` phía `docs/apps-script-kiem-tra.gs` + test dem-tin-moi khoá riêng, không đụng). Muốn gỡ ⇒ xoá luôn cụm test đi kèm; Boss quyết.
- **ĐÃ DỌN (Code 4, 21/09 15:4x):** **D4** (phần NutTaiDeCa) — xoá `NutTaiDeCa.tsx` (235 dòng, thẻ "Tải đề & lời giải" — 0 nơi nhập) + `tests/tai-de-ca-so-cau.test.tsx` + `tests/de-mot-ca-mot-de.test.tsx` (cả hai chỉ khoá thẻ này); `tests/de-rieng-va-so-cau.test.tsx` cắt khối `tải đề riêng của một em`, GIỮ phần link phiếu `~<n>` + ảnh lời giải (là hàm `phieu-link`/`html-phieu` còn dùng). Không mã chết mới lộ: `NutPhieuHtml`, `assignStudentQuestions`, `cauLuyenTu*`, `mergeKeepAnswers` đều còn nơi nhập. `tsc -b`/server sạch. **CHƯA làm (có chủ ý):** `LogoDDH` (6 dòng, test nằm lẫn trong `logo-bo-moi-1909` của làn logo — lãi quá nhỏ), `Spirit3D` (Code 5), `KhoiLuyenKhacPhuc` (đợi dòng "Đã nộp phiếu khắc phục N/M em" ở Theo dõi ca — cần nguồn `khac_phuc` trong `/gv/em-toan-canh` của Code 3).
- **CHỜ LÔ SAU (Boss chốt 21/09 15:4x, Code 4 ghi sổ):** cụm mồ côi lộ ra sau D3 — `src/lib/tro-ly/{nguon,y-dinh,tra-loi,ai-giai-bai,he-thong-chat}.ts` + trong `lib/exam-api.ts` các hàm `listParentMessages`, `markMessagesRead`, `demTinMoi`, kiểu `ParentMessage` — ĐỂ LẠI hôm nay. Điều kiện gỡ: (1) xoá KÈM cụm test đi cùng (`tro-ly`, `ai-giai-bai-1309`, `he-thong-chat-thong-nhau`, phần Apps Script của `dem-tin-moi` nếu lệnh phía máy chủ cũng bỏ); (2) Code 3 xác nhận các lệnh máy chủ tương ứng (`listMessages`, `demTinMoi`, đánh dấu đã đọc, tin nhắn học sinh gửi) cũng KHÔNG còn ai gọi.
- **ĐÃ DỌN (Code 4, 21/09 19:5x):** **LÔ SAU** (a1d9c7d) — xoá `src/lib/tro-ly/{nguon,y-dinh,tra-loi,ai-giai-bai,he-thong-chat}.ts` + 3 test chỉ khoá chúng (`tro-ly`, `ai-giai-bai-1309`, `he-thong-chat-thong-nhau`) + 4 hàm/kiểu tin nhắn ở `exam-api.ts` (`ParentMessage`, `listParentMessages`, `demTinMoi`, `markMessagesRead`): 0 nơi nhập/gọi; lệnh máy chủ không đụng; −1.750 dòng; `tsc -b` 0. **Bản 3 (f0d7fb9)** — ô "Bài tập về nhà đúng nhịp" đọc `nhip.btvnDungNhip` của chính `/gv/bang-tin`, bỏ lệnh cũ `layHomNay` khỏi `HomNayBan3` (HomNayCu bản 2 vẫn giữ). **Còn chờ Code 3:** nguồn `khac_phuc` ở `/gv/em-toan-canh` (rồi mới xoá `KhoiLuyenKhacPhuc.tsx` + `tests/luyen-khac-phuc.test.tsx` — hiện 0 nơi nhập trong `src/`, chỉ còn test); `/gv/bao-cao-ca(+-em)` (`server/src/bao-cao-ca.ts` chưa vào `index.ts`). **Mã chết đang chờ Boss cho gỡ:** gói `opencv-js`/`jspdf` + `roboto-vn`/`xlsx-export`/`engine`/`quality-gate` (lộ ra sau D2), `phan-cau-len-bang.ts` 4 hàm.
