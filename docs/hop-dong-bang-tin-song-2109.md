...
## Màn đã nối (Code 4, cụm e1, 21/09) — điều máy chủ cần biết
- `HomNayScreen` hỏi `POST /gv/bang-tin-song` khi mở màn và **mỗi 10 giây** (dừng khi tab ẩn, hỏi ngay khi hiện lại; không chồng lệnh khi lần trước chưa về). Đang dùng sàn thì màn KHÔNG gọi `/gv/bang-tin` và `/ke-hoach/hom-nay-thay` (lệnh sống đã mang đủ khoá).
- Rơi về **Bảng tin bản 3** (`/gv/bang-tin`, rồi bản 2 nếu lệnh ấy cũng lỗi): 404 (máy chủ cũ), `{ok:false}` (kể cả `lyDo:'tat'`), thân `ok:true` nhưng vắng `song` hoặc `nhip`/mốc sai dạng, hoặc lần đầu mất mạng. Máy chủ nói KHÔNG ⇒ 5 phút sau màn mới hỏi lại lệnh sống; mất mạng / chậm ⇒ hỏi lại ở nhịp 10 giây. Đang xem sàn mà một lần hỏi sau đó mất mạng ⇒ GIỮ số sống đã có.
- Bộ đọc `src/lib/bang-tin-san/doc-san.ts` chống sai kiểu từng khoá: khoá nào sai dạng thì khối ấy ẩn (không bịa 0). `nen: []` ⇒ ẩn khối nến. `song.tia60` cần đủ 3 chuỗi cùng độ dài ≥ 2. `danDau` ≤ 5.
- Tên lớp `"Chưa xếp lớp"` (chữ nội bộ `TEN_LOP_CHUA_XEP`) được đổi thành `"Chưa rõ lớp"` Ở BỘ ĐỌC (mọi khối có tên lớp, kể cả bài tập về nhà và em cần để ý). Máy chủ cứ giữ hằng cũ; test khoá hai chỗ khớp nhau.
- three.js nạp lười (`ban-do-3d-three-*.js` ≈ 536 KB, chỉ máy thầy tải khi mở màn Hôm nay) và `vite.config.ts` bỏ nó khỏi precache: gói chính 938.637 → 938.650 byte (gzip 268.229 → 268.233), precache vẫn 140 mục (2.703 KiB).
