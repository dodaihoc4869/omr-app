...
- **BẮT BUỘC token học sinh** (như Mom nộp bài): `{ "token": "<token HS>", "traLoi": [ { "qid": "…", "dapAn": "B", "giay": 42 }, … ] }`, tối đa **20** mục. SBD trần (không token) và token phụ huynh bị từ chối; `sbd` trong body bị bỏ qua (không nộp hộ em khác). `dapAn`: Phần I `A`–`D`; Phần II bốn ký tự `Đ`/`S` (có dấu hay không đều được; ô bỏ trống ghi `-`); Phần III số (dấu phẩy hay dấu chấm đều được). `dapAn` rỗng hoặc chỉ toàn `-` = BỎ TRỐNG. `giay` tuỳ chọn, hợp lệ 5..1200 thì ghi (làm tròn), ngoài khoảng thì bỏ.
...
- Máy chủ CHẤM (`isAnswerCorrect`, cùng luật chấm của sổ) và ghi sổ `su_kien_hoc` `nguon='on_lai'`, `ma_nguon='on_lai:<ngày VN>'`. **Nộp lại cùng câu cùng ngày giữ kết quả LẦN ĐẦU** (không cho sửa đáp án sau khi đã thấy lời giải; phản hồi lần hai vẫn ra kết quả lần đầu). Bỏ trống → `ket_qua` NULL, không cộng lần sai. Sau đó hồ sơ được dựng lại theo luật Leitner (mốc 1/3/7 ngày; đúng cùng ngày chỉ tính một mốc).
- **Phản hồi** `{ ok: true, ketQua: [...], khongCo: [...], tienBo, exp }`. `ketQua[i]` = `{ qid, dung: true|false|null, dapAnDung, loiGiai, anhLoiGiai: [{viTri:'sau_loi_giai', …}] }` (`null` = bỏ trống; `loiGiai` có thể là chữ hoặc đối tượng như kho, `null` nếu kho chưa có). `tienBo` = `{ daLamCau, lenBac, tutBac }` của HÔM NAY sau lượt nộp (`null` nếu không tính được). `exp` = EXP học tập vừa cộng (0 nếu em chưa có hồ sơ game, đã nhận rồi, hoặc hết trần 100/ngày).
