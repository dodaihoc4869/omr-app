...
- **THẦY LỆNH THÊM 21/09 (Boss chốt cách làm):** (1) "RESET lại hết mảnh khiên và mốc ban đầu cho công bằng": khoá `cau_hinh.khien_moc = '2026-09-21'` — mảnh chỉ tính từ dòng `manh_khien_so` có `ngay_vn` ≥ mốc **và `loai = 'dat'`** (dòng `dang`/`chuoi7` sinh trước lệnh nằm trong ngày mốc không cộng lại); dòng cũ giữ nguyên trong sổ. Hồ sơ game: mọi em `khienRen.manh = 0`, khiên rèn CHƯA dùng = 0 (giữ phần đã dùng), bản chép cũ ở `khien_truoc_reset_2109` (lùi được); KHÔNG đụng EXP, cấp, thần thú, chuỗi ngày, vé, điểm. Một tin cho mỗi em: "A.I Đỗ Đại Học báo em: từ hôm nay khiên được rèn theo luật mới cho mọi bạn…". (2) **Khiên QUÀ tiến hoá ĐẦU (mốc cấp 10) chỉ mở khi có ≥ 36 ngày đạt từ mốc** (phương án B; `khienQuaTienHoa` ở `exp-ho-so-game.ts`, `expMoi.ngayDat`): các mốc tiến hoá và ảnh thú không đổi; máy chủ là nguồn duy nhất của "khiên còn lại". Ước lượng cũ: lên cấp 10 cần 1 630 EXP ≈ 14–19 ngày < 36 nên phải khoá. (3) **Vắng `KHIEN_MAT_KHI_VANG_NGAY = 7` ngày liên tiếp không đạt nhiệm vụ ngày ⇒ mất 1 khiên** (Boss chốt số; thầy đổi ở `exp-cau-hinh.ts`): khiên rèn chưa dùng trước, hết thì khiên quà chưa dùng, không có thì không trừ, không âm; mảnh không bị trừ; ngày nghỉ hợp lệ không tính là vắng; sau khi trừ đếm lại từ 0; cron 00:01 (`server/src/khien-mat.ts`), khoá sổ `khien|mat|<sbd>|<ngày>` (bảng `khien_mat_so`); báo em đúng ngày vắng thứ 5 (chỉ em đang có khiên) và khi mất. Thứ tự chạy thật + câu kiểm + lùi: `docs/chay-that-2109-khien-thu-tu.md`.
